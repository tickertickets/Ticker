import { pgTable, text, boolean, timestamp, integer, primaryKey, jsonb, index } from "drizzle-orm/pg-core";
...
export const chainsTable = pgTable("chains", {
  ...
  // latest run). Kept in sync by application code — see lib/db/src/activity.ts.
  lastActivityAt: timestamp("last_activity_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [
  index("chains_last_activity_at_idx").on(table.lastActivityAt),
]);
