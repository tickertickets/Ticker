...
// ── User blocking ──────────────────────────────────────────────────────────
// blockerId blocked blockedId: standard semantics — blocker no longer sees
// blockedId's content in feeds and vice versa (bidirectional hide), any
// existing follow relationship in either direction is removed, and neither
// side can (re-)follow the other while the block exists.
...
export const reportReasonEnum = pgEnum("report_reason", ["spam", "inappropriate", "harassment", "other", "copyright_infringement"]);
...
export const characterBookmarksTable = pgTable("character_bookmarks", {
  userId: text("user_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
