...
export async function fetchMoodMovies(
  ...
  if (cfg.limit) {
    // Always fetch 3 pages starting from page 1 to guarantee a full pool
    // (3 × 20 = up to 60 items before dedup). Individual fetches are wrapped
    // in catch so a missing page never collapses the whole batch.
    const POOL_PAGES = Math.max(3, Math.ceil(cfg.limit / 20) + 1);
