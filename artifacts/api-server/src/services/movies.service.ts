...
/**
 * Fetch a batch of raw TMDB pages for a curated mood and normalise/dedupe them,
 * WITHOUT slicing to a fixed pool size. Used by the mood route to grow a
 * curated pool on demand as the user keeps scrolling, instead of capping
 * every curated mood at a hard-coded ~100 items.
 *
 * Returns `tmdbTotalPages` (as reported by TMDB for this query) so the caller
 * knows when the underlying dataset is genuinely exhausted vs. just not
 * fetched yet.
 */
export async function fetchMoodPoolBatch(
  cfg: MoodConfig,
...
): Promise<{ movies: NormalizedMovie[]; tmdbTotalPages: number }> {
  const fetches = await Promise.all(
    Array.from({ length: pageCount }, (_, i) =>
      tmdbFetchFromUrl(applyLang(urlFn(fromTmdbPage + i), lang)).catch(
        () => ({ results: [] }) as PagedResult,
      ),
    ),
  );

  const tmdbTotalPages = fetches.reduce(
    (max, d) => Math.max(max, d.total_pages ?? 0),
    0,
  );
  ...
  // Same deterministic per-day shuffle as the original pool builder, applied
  // within this batch so growth pages don't come back in raw TMDB order.
  ...
  const movies = shuffled.map((m) => normalizeItem(m, cfg.mediaTypeA, collMap));
  return { movies, tmdbTotalPages };
}
