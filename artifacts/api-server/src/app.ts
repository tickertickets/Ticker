...
async function ensureBadgeXpLogTable(): Promise<void> {
  try {
    ...
    // Ensure badge_xp_action enum exists (drizzle push creates it, but just in
    // case), then make sure it has the values badge.service.ts actually
    // writes today: tag_ticket / party_complete (tag_friend / party_accept
    // are legacy values from before a rename — kept for old rows, never
    // written by current code; see comment in schema/badges.ts).
    await pool.query(`
      DO $ BEGIN
      ...
      END $;
    `);
    await pool.query(`ALTER TYPE "badge_xp_action" ADD VALUE IF NOT EXISTS 'tag_ticket';`);
    await pool.query(`ALTER TYPE "badge_xp_action" ADD VALUE IF NOT EXISTS 'party_complete';`);
    await pool.query(`
...
async function ensureUserBadgeTable(): Promise<void> {
  try {
    // Ensure badge_xp_action enum exists, then has the values badge.service.ts
    // actually writes today (tag_ticket / party_complete) — see comment above.
    await pool.query(`
      DO $ BEGIN
        CREATE TYPE "badge_xp_action" AS ENUM (
          'post_ticket', 'post_chain', 'tag_friend', 'party_accept'
...
// Render (and most PaaS) sit behind 2+ proxy hops.
// "true" trusts the entire X-Forwarded-For chain set by Render's infrastructure,
// so req.ip always resolves to the real client IP instead of Render's CDN IP.
...
// This server is a JSON API (the actual browser-rendered app is the static
// frontend on Vercel, which sets its own CSP in vercel.json). We still lock
// this down: a strict CSP for any accidental HTML/error responses, HSTS, and
// standard anti-sniffing/clickjacking headers, without breaking JSON responses
// or the robots.txt/sitemap.xml routes served below.
app.use(
  helmet({
...
app.get("/sitemap.xml", async (_req, res) => {
  ...
  const BASE = "https://ticker-tickets.vercel.app";
  const today = new Date().toISOString().slice(0, 10);

  let chainUrls = "";
  try {
    const { rows } = await pool.query<{ id: string; updated_at: string }>(
      `SELECT id, updated_at FROM chains
       WHERE is_private = false
       ORDER BY chain_count DESC, updated_at DESC
       LIMIT 200`
    );
...
// When deployed on Replit, the ticker-web build output lives at
// artifacts/ticker-web/public relative to the workspace root.
// __dirname inside dist/index.mjs → artifacts/api-server/dist/
const frontendDist = path.resolve(__dirname, "../../../ticker-web/public");
