...
export function initSocket(httpServer: HTTPServer): SocketIOServer {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("wiki:leave", (wikiPageId: string) => {
      socket.leave(`wiki:${wikiPageId}`);
    });
...
export function emitCommentUpdated(ticketId: string): void {
  io?.to(`ticket:${ticketId}`).emit("comment:updated", { ticketId });
}
...
export function emitChainCommentNew(chainId: string): void {
  io?.to(`chain:${chainId}`).emit("chain:comment:new", { chainId });
}

export function emitChainCommentDeleted(chainId: string): void {
  io?.to(`chain:${chainId}`).emit("chain:comment:deleted", { chainId });
}

export function emitChainCommentUpdated(chainId: string): void {
  io?.to(`chain:${chainId}`).emit("chain:comment:updated", { chainId });
}

export function emitWikiLiked(wikiPageId: string, likeCount: number): void {
  io?.to(`wiki:${wikiPageId}`).emit("wiki:liked", { wikiPageId, likeCount });
}

export function emitWikiCommentNew(wikiPageId: string): void {
  io?.to(`wiki:${wikiPageId}`).emit("wiki:comment:new", { wikiPageId });
}

export function emitWikiCommentDeleted(wikiPageId: string): void {
  io?.to(`wiki:${wikiPageId}`).emit("wiki:comment:deleted", { wikiPageId });
}

export function emitBadgeUpdate(userId: string): void {
  io?.to(`user:${userId}`).emit("badge:update", { userId });
}
