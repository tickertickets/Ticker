...
import { awardPartyCompleteXp } from "../services/badge.service";
...
router.post("/invite/:inviteId/accept", async (req, res) => {
  ...
  const { inviteId } = req.params;
  const { seatNumber, memoryNote, watchedAt, location, rating, ratingType, isPrivate, hideWatchedAt, hideLocation, hideRating } = req.body;
  ...
  await db.insert(ticketsTable).values({
    ...
    hideRating: typeof hideRating === "boolean" ? hideRating : false,
  ...
  // Badge XP: award party_complete XP only when ALL invitees have accepted.
  if (invite.inviterUserId !== currentUserId) {
    const [nonAccepted] = await db
      .select({ count: sql<number>`count(*)::int` })
      .from(partyInvitesTable)
      .where(and(
        eq(partyInvitesTable.partyGroupId, partyGroupId),
        ne(partyInvitesTable.status, "accepted"),
      ));
    if (Number(nonAccepted?.count ?? 1) === 0) {
      // All accepted — count total invitees for group-size bonus (+1 XP/person if ≥3)
      const [totalRow] = await db
        .select({ count: sql<number>`count(*)::int` })
        .from(partyInvitesTable)
        .where(eq(partyInvitesTable.partyGroupId, partyGroupId));
      awardPartyCompleteXp(
