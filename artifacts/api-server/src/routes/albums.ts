...
router.get("/", async (req, res) => {
  ...
  const whereCondition = isOwner
    ? and(eq(albumsTable.userId, userId), isNull(albumsTable.archivedAt))
    : and(eq(albumsTable.userId, userId), isNull(albumsTable.archivedAt), eq(albumsTable.isHidden, false));
...
router.post("/", async (req, res) => {
  ...
  // Max 10 albums per user total, counting the implicit "Main" album that
  // every user has — so at most 9 non-archived custom albums may exist here.
  // Archived albums don't count since they're no longer active/visible.
  const existing = await db.select({ id: albumsTable.id }).from(albumsTable)
    .where(and(eq(albumsTable.userId, currentUserId), isNull(albumsTable.archivedAt)));
  if (existing.length >= 9) {
    res.status(400).json({ error: "album_limit_reached" });
    return;
  }
...
// ── PATCH /albums/:id/hide — toggle album visibility (owner only) ────────────
// NOTE: must be declared BEFORE "/:id" so Express doesn't treat "hide" as an id
router.patch("/:id/hide", async (req, res) => {
  ...
  const nextHidden = !album.isHidden;
  await db.update(albumsTable).set({ isHidden: nextHidden, updatedAt: new Date() }).where(eq(albumsTable.id, id));
  res.json({ success: true, isHidden: nextHidden });
});

// ── PATCH /albums/reorder — reorder albums by display order ──────────────────
// NOTE: must be declared BEFORE "/:id" so Express doesn't treat "reorder" as an id
