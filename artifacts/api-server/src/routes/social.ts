...
import { db, bumpTicketActivity, recomputeTicketActivity } from "@workspace/db";
import {
...
import { emitTicketLiked, emitCommentNew, emitCommentDeleted, emitCommentUpdated } from "../lib/socket";
...
router.post("/:ticketId/react", async (req, res) => {
  ...
  if (stats.hasReacted) {
    await bumpTicketActivity(db, ticketId);
  } else {
    await recomputeTicketActivity(db, ticketId);
  }
...
// ── PATCH /:ticketId/comments/:commentId — edit own comment ──────────────────
router.patch("/:ticketId/comments/:commentId", async (req, res) => {
  ...
  const clean = sanitize(content.trim());
  await db.update(commentsTable).set({ content: clean }).where(eq(commentsTable.id, commentId));
  emitCommentUpdated(ticketId);
...
router.post("/:ticketId/report", async (req, res) => {
  ...
  // DB enum รองรับเฉพาะ spam/inappropriate/harassment/other
  // → copyright_infringement ให้ fallback เป็น "other" แต่เก็บ reason จริงใน details
  const isCopyright = reason === "copyright_infringement";
  const dbReason = isCopyright ? "other" : (["spam", "inappropriate", "harassment", "other"].includes(reason) ? reason : "other");
  let storedDetails = details ?? null;
  if (isCopyright) {
    const parts = ["[DMCA Copyright Infringement]"];
    if (details) parts.push(details);
    if (email) parts.push(`Email: ${email}`);
    storedDetails = parts.join("\n");
  } else if (email) {
  ...
  await db.insert(reportsTable).values({
    ...
    ticketId,
    reason: dbReason,
    details: storedDetails ? sanitize(storedDetails) : null,
  });
