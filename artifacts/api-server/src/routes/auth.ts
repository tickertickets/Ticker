...
// Login per-IP: 40 ครั้ง / 15 นาที / IP — นับเฉพาะที่ล้มเหลว
// เพิ่มจาก 10 → 40 เพื่อรองรับ shared-WiFi (บ้าน/หอพัก) ที่มีผู้ใช้หลายคน
// attacker ยังติดกำแพงนี้แม้จะ rotate X-Device-ID
...
// Login per-device: 10 ครั้ง / 15 นาที / เครื่อง — นับเฉพาะที่ล้มเหลว
// ทำให้ผู้ใช้แต่ละคนใน WiFi เดียวกันได้รับ limit แยกกัน
// ถ้าไม่มี X-Device-ID (เช่น bot/API) ให้ fall back ไป IP key
const authDeviceLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 10,
  keyGenerator: (req) => {
    ...
    return `ip:auth-device:${ipKeyGenerator(req as any)}`;
  },
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: "too_many_requests", message: "เกิดข้อผิดพลาด กรุณาลองใหม่ในภายหลัง" },
...
// skipSuccessfulRequests: true — ผู้ใช้ที่สมัครสำเร็จไม่นับเข้า counter
// (เช่นเดียวกับ authIpLimiter) มิฉะนั้น 20 คนในมหาลัยสมัครสำเร็จ → ทุกคนหลังจากนั้นโดนบล็อก
...
// skipSuccessfulRequests: true — สมัครสำเร็จไม่นับ มิฉะนั้น 5 ครั้งสมัครสำเร็จ → ล็อคทันที
const signupDeviceLimiter = rateLimit({
  ...
  keyGenerator: (req) => {
    ...
    return `device:signup:ip:${ipKeyGenerator(req as any)}`;
  },
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: true,
...
router.post("/login", authIpLimiter, authDeviceLimiter, async (req, res) => {
...
router.post("/logout", async (req, res) => {
  ...
  // Clear the cookie regardless of what happens below so the browser always
  // loses its session reference. Options must match the cookie as originally
  // set (SameSite=None; Secure) — omitting them causes browsers to silently
  // ignore the deletion for cross-origin / proxied setups.
  const cookieOpts = {
    httpOnly: true,
  ...
  // Belt-and-suspenders: delete ALL sessions for this userId directly from
  // the DB. This is the most reliable path — it works even if session.destroy()
  // fails (e.g. a DB blip) and cleans up any other open sessions the user has.
  if (userId) {
    try {
      await pool.query(
        `DELETE FROM "user_sessions" WHERE (sess::jsonb)->>'userId' = $1`,
      ...
      req.log.warn({ dbErr }, "logout: failed to delete sessions by userId");
    }
  }

  // Also call session.destroy() to clean up the in-memory session store entry
  // and guarantee the current sid is gone, regardless of the bulk delete above.
  req.session.destroy((err) => {
    if (err) {
      req.log.warn({ err }, "logout: session.destroy() failed (non-fatal)");
    }
    res.clearCookie("ticker_session", cookieOpts);
