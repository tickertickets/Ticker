...
async function fetchPopularFictionItems(): Promise<PopularFictionItem[]> {
  ...
  // @ts-ignore — unreachable reference code, popularCache narrowing is intentional
  if (popularCache && popularCache.date === todayUtc) return popularCache.items;
  // @ts-ignore — unreachable reference code, popularFetchPromise non-null assertion intended
      ...
      await Promise.all(
        Array.from({ length: Math.ceil(topArticles.length / BATCH) }, (_, i) =>
          ...
          try {
            ...
            for (const page of Object.values(((ppData as any).query?.pages ?? {}) as Record<string, any>)) {
              if (page.title) {
      ...
      await Promise.all(
        Array.from({ length: Math.ceil(allQids.length / BATCH) }, (_, i) =>
          ...
          try {
            ...
            for (const [qid, ent] of Object.entries(((entData as any).entities ?? {}) as Record<string, WdEntityData>)) {
...
router.get("/search", async (req, res) => {
  ...
  // eslint-disable-next-line no-unreachable
  // @ts-ignore — unreachable reference code below this point
  ...
  // @ts-ignore — unreachable reference code
  const cached = searchCache.get(cacheKey);
  // @ts-ignore — unreachable reference code
  if (cached && Date.now() - (cached as any).ts < SEARCH_CACHE_TTL) {
    // @ts-ignore — unreachable reference code
    res.json({ items: (cached as any).items.slice(0, limit) });
    return;
  }
  ...
  if (inflight) {
    try {
      // @ts-ignore — unreachable reference code
      const items = await (inflight as Promise<unknown[]>);
      res.json({ items: (items as any[]).slice(0, limit) });
    } catch {
      res.json({ items: [] });
    }
    return;
  ...
  try {
    ...
    res.json({ items: stale?.items?.slice(0, limit) ?? [] });
  }
});
