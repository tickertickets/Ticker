...
import { db, bumpChainActivity, recomputeChainActivity } from "@workspace/db";
import {
  usersTable,
  chainsTable,
  chainMoviesTable,
...
import { hotScore, makeFreshBoost, applyDiversityCap, applyDiversitySpread, paginateRanked, DIVERSITY_CAP, AFFINITY_FOLLOWED } from "../lib/hot-score";
...
// ── Chain creation rate limiters (layered — all must pass) ────────────────────
//
// Chains take more effort to create than single tickets (curated movie lists),
// so limits are slightly lower while still generous:
//   3 per 10 min — burst guard; prevents batch-creating spammy lists
//   20 per day   — absolute daily cap for healthy curation frequency

const chainDailyLimiter = rateLimit({
  ...
  message: { error: "rate_limited", message: "Daily chain limit reached (20/day). Come back tomorrow!" },
});

const chainBurstLimiter = rateLimit({
  ...
  keyGenerator: (req) => req.session?.userId ?? "anon",
  validate: { xForwardedForHeader: false },
  message: { error: "rate_limited", message: "Creating chains too fast. Please wait a few minutes." },
});
...
router.get("/", async (req, res) => {
  ...
  // Offset-based cursor for explore feed pagination (same approach as /api/feed).
  // Timestamp cursors are wrong for score-ranked feeds — a new high-scoring post
  // created between page 1 and page 2 would be silently skipped.
  const cursorParam = req.query["cursor"] as string | undefined;
  const rankedOffset = cursorParam ? Math.max(0, parseInt(cursorParam, 10) || 0) : 0;
  ...
  if (userId) {
    chains = await db.select().from(chainsTable)
      ...
      .orderBy(
        sql`${chainsTable.displayOrder} ASC NULLS FIRST`,
    ...
    // For logged-in users: also include non-private chains from followed private-account users.
    //
    // Pool must cover all pages up to and including the requested offset.
    // Minimum = limit * 6 to handle page 1 well even with thin content.
    const POOL = Math.max(limit * 6, (rankedOffset + limit) * 3);

    // Get followed user IDs (for private account inclusion)
    ...
    const followedSet = new Set([...followedIds, ...(currentUserId ? [currentUserId] : [])]);

    // Build visibility condition: public accounts OR (logged-in user follows them OR it's own content)
    const alwaysIncludeIds = [...followedIds, ...(currentUserId ? [currentUserId] : [])];
    const userVisibleCondition = alwaysIncludeIds.length > 0
      ? or(eq(usersTable.isPrivate, false), inArray(chainsTable.userId, alwaysIncludeIds))
      : eq(usersTable.isPrivate, false);

    const rawRows = await db.select({ chain: chainsTable }).from(chainsTable)
      .innerJoin(usersTable, eq(chainsTable.userId, usersTable.id))
      .where(and(isNull(chainsTable.deletedAt), eq(chainsTable.isPrivate, false), userVisibleCondition))
    ...
    if (poolChains.length > 0) {
      ...
      // Home mode for logged-in users: followed users also get fresh boost.
      // Discover/explore mode (no auth): only own fresh posts are boosted.
      const freshBoostFn = makeFreshBoost(followedSet.size > 1 ? followedSet : null, currentUserId);
      ...
      // Applied across the FULL pool so pagination is stable — same author distribution
      // on page 1 and page 2 as on any other page.
      // Proportional per-user cap: prevents monopolisation in any page-sized window.
      const pagesInPool = Math.max(1, Math.ceil(scored.length / limit));
      const poolCapPerUser = pagesInPool * DIVERSITY_CAP;
      const effectiveChainCap = scored.length <= limit ? scored.length : poolCapPerUser;
      const spread = applyDiversitySpread(scored, (s) => s.chain.userId, effectiveChainCap, scored.length);
  ...
  // Paginate — userId mode uses simple hasMore/limit slice, explore uses offset cursor.
  // Explore (no userId) draws from everyone — like a TikTok/Instagram Explore
  // feed it recycles the ranked list (re-scored fresh every request) instead
  // of hard-stopping once genuinely new candidates run out. A specific user's
  // profile chain list is a bounded, finite collection — it should end normally.
  let items: typeof chains;
  let hasMore: boolean;
  let nextCursor: string | null;
  let recycled = false;

  if (userId) {
    hasMore = chains.length > limit;
    items = chains.slice(0, limit);
    nextCursor = null; // profile chain lists don't paginate via cursor
  } else {
    const paginated = paginateRanked(chains, rankedOffset, limit, true);
  ...
  res.json({ chains: result, hasMore, nextCursor, recycled });
});
...
router.get("/runs", async (req, res) => {
  ...
  // Ordering strategy:
  // 1. Runs with no displayOrder (null) appear FIRST, sorted newest-first.
  //    This ensures new runs always surface at the top even when older runs have saved order.
  // 2. Runs with a saved displayOrder appear after, in their saved order.
  // Fallback: if display_order column doesn't exist yet (pre-migration), order by started_at only.
  let runs: typeof chainRunsTable.$inferSelect[];
  try {
    runs = await db.select().from(chainRunsTable)
      .where(eq(chainRunsTable.userId, userId))
      .orderBy(
        sql`CASE WHEN ${chainRunsTable.displayOrder} IS NULL THEN 0 ELSE 1 END`,
        asc(chainRunsTable.displayOrder),
        desc(chainRunsTable.startedAt),
      )
      .limit(20);
  } catch {
    // display_order column not yet present in this environment — fall back to simple ordering
    runs = await db.select({
      id: chainRunsTable.id,
      chainId: chainRunsTable.chainId,
      userId: chainRunsTable.userId,
      status: chainRunsTable.status,
      totalElapsedMs: chainRunsTable.totalElapsedMs,
      completedCount: chainRunsTable.completedCount,
      startedAt: chainRunsTable.startedAt,
      updatedAt: chainRunsTable.updatedAt,
      displayOrder: sql<number | null>`null`,
    } as any).from(chainRunsTable)
      .where(eq(chainRunsTable.userId, userId))
      .orderBy(desc(chainRunsTable.startedAt))
      .limit(20) as any;
  }
  ...
  const visibleChainIds = chains
    .filter(c => !blockedOwnerIds.has(c.userId) && (currentUserId === userId || !c.isPrivate))
    .map(c => c.id);
  ...
  const result = runs.map(run => {
    ...
    // Also hide private chains from non-owners
    if (chain.isPrivate && chain.userId !== currentUserId) return null;
...
// ── GET /chains/search — keyword search ──────────────────────────────────────
router.get("/search", async (req, res) => {
  ...
  const chains = await db
    .select({
      ...
      descriptionAlign: chainsTable.descriptionAlign,
      chainCount: chainsTable.chainCount,
      isPrivate: chainsTable.isPrivate,
      hideLikes: chainsTable.hideLikes,
      hideComments: chainsTable.hideComments,
      hideChainCount: chainsTable.hideChainCount,
      mode: chainsTable.mode,
      challengeDurationMs: chainsTable.challengeDurationMs,
      createdAt: chainsTable.createdAt,
      updatedAt: chainsTable.updatedAt,
    ...
    .where(
      and(
        ...
        eq(usersTable.isPrivate, false),
        or(
          ilike(chainsTable.title, `%${term}%`),
          ilike(chainsTable.description, `%${term}%`),
        ),
      ),
    )
  ...
  const [allMovies, likeCounts, commentCounts, runCounts, userLikes, userBookmarks] = await Promise.all([
    db.select({
      chainId: chainMoviesTable.chainId,
      posterUrl: chainMoviesTable.posterUrl,
      genre: chainMoviesTable.genre,
    })
      .from(chainMoviesTable)
      .where(inArray(chainMoviesTable.chainId, chainIds))
      .orderBy(asc(chainMoviesTable.position)),
  ...
  const moviesByChain = new Map<string, { posterUrl: string | null; genre: string | null }[]>();
  for (const m of allMovies) {
    if (!moviesByChain.has(m.chainId)) moviesByChain.set(m.chainId, []);
    moviesByChain.get(m.chainId)!.push({ posterUrl: m.posterUrl ?? null, genre: m.genre ?? null });
  }
  const likeCountMap = new Map(likeCounts.map(r => [r.chainId, Number(r.cnt)]));
  const commentCountMap = new Map(commentCounts.map(r => [r.chainId, Number(r.cnt)]));
  const runCountMap = new Map(runCounts.map(r => [r.chainId, Number(r.cnt)]));
  const likedSet = new Set((userLikes as { chainId: string }[]).map(r => r.chainId));
  const bookmarkedSet = new Set((userBookmarks as { chainId: string }[]).map(r => r.chainId));
  ...
  res.json({
    chains: chains.map(c => ({
      ...
      chainCount: runCountMap.get(c.id) ?? Number(c.chainCount),
      movieCount: moviesByChain.get(c.id)?.length ?? 0,
...
router.get("/:chainId", async (req, res) => {
  ...
  // ?viewUserId=xxx — passed when navigating from another user's "เล่นแล้ว" tab
  const viewUserId = typeof req.query.viewUserId === "string" ? req.query.viewUserId : undefined;
  ...
  // Attach the viewed user's run when ?viewUserId= is set and differs from caller
  let viewedUserRun = null;
  if (viewUserId && viewUserId !== currentUserId) {
    const [vRun] = await db.select().from(chainRunsTable)
      .where(and(eq(chainRunsTable.chainId, chainId), eq(chainRunsTable.userId, viewUserId)))
      .orderBy(desc(chainRunsTable.startedAt))
      .limit(1);
    if (vRun) {
      const [vItems, rawMovies, vUserRows] = await Promise.all([
        db.select().from(chainRunItemsTable)
          .where(eq(chainRunItemsTable.runId, vRun.id))
          .orderBy(asc(chainRunItemsTable.position)),
        db.select().from(chainMoviesTable)
          .where(eq(chainMoviesTable.chainId, chainId))
          .orderBy(asc(chainMoviesTable.position)),
        db.select().from(usersTable).where(eq(usersTable.id, viewUserId)).limit(1),
      ]);
      viewedUserRun = buildRunSummary(vRun, vItems, rawMovies, vUserRows[0]);
    }
  }

  res.json({ ...result, viewedUserRun });
});

// ── POST /chains — create chain ────────────────────────────────────────────────
router.post("/", chainBurstLimiter, chainDailyLimiter, async (req, res) => {
  const currentUserId = req.session?.userId;
  if (!currentUserId) { res.status(401).json({ error: "unauthorized" }); return; }
...
// ── PATCH /chains/runs/reorder — user reorders their played chain runs ────────
// NOTE: registered before /:chainId to avoid route conflicts.
router.patch("/runs/reorder", async (req, res) => {
  const currentUserId = req.session?.userId;
  if (!currentUserId) { res.status(401).json({ error: "unauthorized" }); return; }
  const { runIds } = req.body as { runIds: string[] };
  if (!Array.isArray(runIds) || runIds.length === 0) {
    res.status(400).json({ error: "runIds must be a non-empty array" }); return;
  }
  await Promise.all(
    runIds.map((id, idx) =>
      db.update(chainRunsTable)
        .set({ displayOrder: idx + 1 })
        .where(and(eq(chainRunsTable.id, id), eq(chainRunsTable.userId, currentUserId)))
    )
  );
  res.json({ success: true });
});

// ── PATCH /chains/reorder — owner reorders their created chains ───────────────
// NOTE: MUST be registered BEFORE router.patch("/:chainId", ...) to avoid
//       Express treating "reorder" as a chainId value.
router.patch("/reorder", async (req, res) => {
  const currentUserId = req.session?.userId;
  if (!currentUserId) { res.status(401).json({ error: "unauthorized" }); return; }
  const { chainIds } = req.body as { chainIds: string[] };
  if (!Array.isArray(chainIds) || chainIds.length === 0) {
    res.status(400).json({ error: "chainIds must be a non-empty array" }); return;
  }
  await Promise.all(
    chainIds.map((id, idx) =>
      db.update(chainsTable)
        .set({ displayOrder: idx + 1 })
        .where(and(eq(chainsTable.id, id), eq(chainsTable.userId, currentUserId)))
    )
  );
  res.json({ success: true });
});
...
router.patch("/:chainId", async (req, res) => {
  ...
  const updateData: Partial<typeof chainsTable.$inferInsert> = {
    ...
    descriptionAlign: (descriptionAlign === "left" || descriptionAlign === "center" || descriptionAlign === "right") ? descriptionAlign : chain.descriptionAlign ?? "left",
  };
...
router.post("/:chainId/run", async (req, res) => {
  ...
  await bumpChainActivity(db, chainId);
...
router.delete("/:chainId/run/:runId", async (req, res) => {
  ...
  const [run] = await db.select().from(chainRunsTable)
    .where(and(eq(chainRunsTable.id, runId), eq(chainRunsTable.userId, currentUserId))).limit(1);
  if (!run) { res.status(404).json({ error: "not_found" }); return; }
  // Allow cancelling both live and completed runs (user may want to restart)
  if (run.status !== "live" && run.status !== "completed") {
  ...
  await recomputeChainActivity(db, chainId);

  res.json({ success: true });
});

// ── PATCH /chains/:chainId/run/:runId/item/:itemId/note — edit review note ────
// Allows editing the memoryNote on an already-finished run item.
router.patch("/:chainId/run/:runId/item/:itemId/note", async (req, res) => {
  const currentUserId = req.session?.userId;
  if (!currentUserId) { res.status(401).json({ error: "unauthorized" }); return; }
  const { runId, itemId } = req.params;
  ...
  await db.update(chainRunItemsTable).set({
    memoryNote: memoryNote ? sanitize(String(memoryNote).slice(0, 150)) : null,
    updatedAt: new Date(),
  }).where(and(eq(chainRunItemsTable.id, itemId), eq(chainRunItemsTable.runId, runId)));
...
router.post("/:chainId/like", async (req, res) => {
  ...
  let inserted = false;
  try {
    const result = await db.insert(chainLikesTable).values({ userId: currentUserId, chainId }).onConflictDoNothing();
    inserted = (result.rowCount ?? 0) > 0;
    if (inserted) await bumpChainActivity(db, chainId);
  } catch (err) {
    console.warn("[chains] like insert error (non-fatal):", (err as Error).message);
  }
  // Notify chain owner (best-effort, skip self-likes)
  if (inserted) {
    try {
      const [chain] = await db.select({ userId: chainsTable.userId, title: chainsTable.title })
        .from(chainsTable).where(eq(chainsTable.id, chainId)).limit(1);
      if (chain && chain.userId !== currentUserId) {
...
router.get("/:chainId/comments", async (req, res) => {
  ...
  const comments = await db.select({
    ...
    replyToId: chainCommentsTable.replyToId,
    username: usersTable.username,
    displayName: usersTable.displayName,
    avatarUrl: usersTable.avatarUrl,
  })
...
router.delete("/:chainId/comments/:commentId", async (req, res) => {
  ...
  await recomputeChainActivity(db, chainId);
  try {
    const { emitChainCommentDeleted } = await import("../lib/socket");
    emitChainCommentDeleted(chainId);
  } catch { /* best-effort */ }
  res.json({ success: true });
});

// ── PATCH /chains/:chainId/comments/:commentId — edit own chain comment ───────
router.patch("/:chainId/comments/:commentId", async (req, res) => {
  const currentUserId = req.session?.userId;
  if (!currentUserId) { res.status(401).json({ error: "unauthorized" }); return; }
  const { commentId } = req.params;
  const { content } = req.body;
  if (!content?.trim()) { res.status(400).json({ error: "bad_request", message: "Content required" }); return; }
  const [comment] = await db.select().from(chainCommentsTable).where(eq(chainCommentsTable.id, commentId)).limit(1);
  if (!comment) { res.status(404).json({ error: "not_found" }); return; }
  if (comment.userId !== currentUserId) { res.status(403).json({ error: "forbidden" }); return; }
  const clean = sanitize(content.trim());
  await db.update(chainCommentsTable).set({ content: clean }).where(eq(chainCommentsTable.id, commentId));
  try {
    const { emitChainCommentUpdated } = await import("../lib/socket");
    const { chainId } = req.params;
    emitChainCommentUpdated(chainId);
  } catch { /* best-effort */ }
  res.json({ success: true });
});
...
router.post("/:chainId/comments", async (req, res) => {
  ...
  // Notify chain owner (best-effort, skip self-comments)
  try {
    const [chain] = await db.select({ userId: chainsTable.userId, title: chainsTable.title })
      .from(chainsTable).where(eq(chainsTable.id, chainId)).limit(1);
    if (chain && chain.userId !== currentUserId) {
    ...
    if (validReplyToId) {
      const [parentComment] = await db.select({ userId: chainCommentsTable.userId })
        .from(chainCommentsTable).where(eq(chainCommentsTable.id, validReplyToId)).limit(1);
      const parentAuthorId = parentComment?.userId;
      if (parentAuthorId && parentAuthorId !== currentUserId && parentAuthorId !== chain?.userId) {
        await createNotification({
          id: nanoid(),
          userId: parentAuthorId,
          fromUserId: currentUserId,
          type: "chain_comment_reply",
          chainId,
  ...
  // Notify all viewers in the chain room so comments update in real-time
  try {
    const { emitChainCommentNew } = await import("../lib/socket");
    emitChainCommentNew(chainId);
  } catch { /* best-effort */ }
  res.json({
    id,
    chainId,
...
router.post("/:chainId/report", async (req, res) => {
  ...
  const validReasons = ["spam", "inappropriate", "harassment", "other", "copyright_infringement"] as const;
  ...
  const isCopyright = reason === "copyright_infringement";
  let extraDetails = details ? String(details).slice(0, 500) : null;
  if (isCopyright && email) {
    extraDetails = [extraDetails, `Email: ${email}`].filter(Boolean).join("\n");
  }
