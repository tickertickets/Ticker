...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // ตรวจ dedup: ห้ามรายงาน chain เดิมซ้ำ
    const [alreadyChain] = await db
      .select({ id: reportsTable.id })
      .from(reportsTable)
      .where(and(eq(reportsTable.reporterId, currentUserId), eq(reportsTable.chainId, chainId)))
      .limit(1);
    if (alreadyChain) throw new ConflictError("already_reported", "Already reported this chain");

    const isCopyright = reason === "copyright_infringement";
    const allValidReasons = ["spam", "inappropriate", "harassment", "other", "copyright_infringement"] as const;
    const dbReason: (typeof allValidReasons)[number] = (allValidReasons as readonly string[]).includes(reason)
      ? (reason as (typeof allValidReasons)[number])
      : "other";

    let extraDetails = details ?? null;
    if (isCopyright) {
    ...
    await db.insert(reportsTable).values({
      ...
      chainId,
      reason: dbReason,
      details: extraDetails ? sanitize(extraDetails) : null,
    });

    await notifyReport({
      type: "ticket",
      reason: isCopyright ? "other" : dbReason,
      details: extraDetails ?? undefined,
      reporterUsername: reporter?.username ?? undefined,
      targetId: chainId,
      extraLabel: `Chain ID: ${chainId}${isCopyright ? " | ⚠️ DMCA Takedown" : ""}${isCopyright && email ? ` | Email: ${email}` : ""}`,
    });
