...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // - cache per calendar day so the list changes every 24 h
    // - start from a different TMDB page each day to maximise variety
    if (cfg.limit) {
      const cacheKey = `mood-${moodId}-${subFilter || "main"}-${lang}-${todayStr()}`;
      const cached = await getCached(cacheKey, MOOD_TTL_MS);
      if (cached) {
        const c = cached as { movies: Record<string, any>[] };
        await ensureMovieCores(c.movies);
