...
const TRAILER_TTL_MS      = 1000 * 60 * 60 * 24;  // 24 h — trailers rarely change after release
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const movieRows: any[] = imdbIds.length > 0
      ? await db.select().from(moviesTable).where(inArray((moviesTable as any).imdbId, imdbIds))
      : [];
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // - cache the pool per calendar day so repeat pages are served from one fetch
    // - grow the pool ON DEMAND as the user scrolls further, instead of capping
    //   every curated mood at a hard-coded ~100 items — scrolling only actually
    //   stops once TMDB itself runs out of results for that query.
    // - never persist a pool that came back empty: an empty result almost always
    //   means a transient TMDB hiccup, not a genuinely empty category, and
    //   caching it would show "no movies found" to every user for the next 25h.
    if (cfg.limit) {
      const PAGE_SIZE = 20;
      const GROWTH_TMDB_PAGES = 5;
      const TMDB_MAX_PAGE = 500; // TMDB's own hard page cap
      ...
      if (cachedPool && Array.isArray((cachedPool as any).allMovies) && (cachedPool as any).allMovies.length > 0) {
      ...
      // AND TMDB still has more pages to give — true infinite scroll, capped
      // only by the real dataset size rather than an artificial local limit.
      const neededItems = page * PAGE_SIZE;
      const seenIds = new Set(poolState.allMovies.map((m: any) => m.tmdbId ?? m.id));
      const pageCap = Math.min(poolState.tmdbTotalPages || TMDB_MAX_PAGE, TMDB_MAX_PAGE);
      while (poolState.allMovies.length < neededItems && poolState.tmdbPagesFetched < pageCap) {
        const batch = await fetchMoodPoolBatch(
          cfg, urlFn,
          startPage + poolState.tmdbPagesFetched,
          GROWTH_TMDB_PAGES,
          startPage,
          lang,
        );
        const fresh = (batch.movies as Record<string, any>[]).filter((m) => {
          const id = m.tmdbId ?? m.id;
          if (seenIds.has(id)) return false;
          seenIds.add(id);
          return true;
        });
        poolState.allMovies.push(...fresh);
        poolState.tmdbPagesFetched += GROWTH_TMDB_PAGES;
        if (batch.tmdbTotalPages) poolState.tmdbTotalPages = batch.tmdbTotalPages;
        if (fresh.length === 0) break; // TMDB gave us nothing new — treat as exhausted
      }
      ...
      const pageMovies = poolState.allMovies.slice(start, start + PAGE_SIZE);
      const exhausted = poolState.tmdbPagesFetched >= Math.min(poolState.tmdbTotalPages || TMDB_MAX_PAGE, TMDB_MAX_PAGE);
      const totalPages = exhausted
        ? Math.max(1, Math.ceil(poolState.allMovies.length / PAGE_SIZE))
        : Math.max(page + 1, Math.ceil(poolState.allMovies.length / PAGE_SIZE));

      await ensureMovieCores(pageMovies);
      res.json({ movies: pageMovies, page, totalPages, totalResults: poolState.allMovies.length });
      return;
    }
    ...
    if (topPerson && Number(page) === 1) {
      ...
      try {
        ...
        const crewMovies = (crewData.crew ?? [])
          .filter((m) => (m as any).job === "Director" || !(m as any).job)
          .filter((m) => m.vote_count && m.vote_count > 50)
          .sort((a, b) => (b.popularity ?? 0) - (a.popularity ?? 0))
          .slice(0, 10)
          .map((m) => ({ ...m, media_type: "movie" as const }));
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const chains = taggedChains
      ...
      .sort((a, b) => {
        // hotScore — same ranking engine as /api/feed.
        // bonus = chainCount (distinct users who joined = strongest community signal).
        // lastActivityAt = updatedAt (bumped on new runs, likes, comments).
        const scoreA = hotScore({
          likes: a.likeCount,
          comments: a.commentCount,
          bonus: a.chainCount,
          lastActivityAt: a.updatedAt ? new Date(a.updatedAt) : (a.createdAt ? new Date(a.createdAt) : new Date()),
        });
        const scoreB = hotScore({
          likes: b.likeCount,
          comments: b.commentCount,
          bonus: b.chainCount,
          lastActivityAt: b.updatedAt ? new Date(b.updatedAt) : (b.createdAt ? new Date(b.createdAt) : new Date()),
        });
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Bulk-fetch engagement counts + last activity timestamps + top comment preview (no N+1)
    // NOTE: likes use ticketReactionsTable (weighted score) — NOT the legacy likesTable.
    const COMMUNITY_REACTION_POINTS: Record<string, number> = {
      heart: 1, fire: 2, lightning: 3, sparkle: 4, popcorn: 5,
    };
    const [reactionRows, commentRows, commentPreviewRows] = await Promise.all([
      db
        .select({
      ...
      // Newest top-level comment per ticket — used for preview in the community section
      db
        .select({
        ...
        .where(and(inArray(commentsTable.ticketId, ticketIds), isNull(commentsTable.replyToId)))
        .orderBy(desc(commentsTable.createdAt))
        .limit(ticketIds.length * 2),
    ]);

    // Aggregate reactions → totalScore per ticket (matching tickets.service.ts logic)
    const likeMap       = new Map<string, number>();
    const likeLastAtMap = new Map<string, Date | null>();
    for (const row of reactionRows) {
      const pts = (COMMUNITY_REACTION_POINTS[row.reactionType] ?? 1) * row.count;
      likeMap.set(row.ticketId, (likeMap.get(row.ticketId) ?? 0) + pts);
      const updatedAt = row.updatedAt ? new Date(row.updatedAt) : null;
      const prev = likeLastAtMap.get(row.ticketId) ?? null;
      if (!prev || (updatedAt && updatedAt > prev)) likeLastAtMap.set(row.ticketId, updatedAt);
    }
    ...
    // Pick the newest top-level comment per ticket for preview
    const topCommentMap = new Map<string, { content: string; username: string | null; displayName: string | null }>();
    for (const row of commentPreviewRows) {
      if (!topCommentMap.has(row.ticketId)) {
        topCommentMap.set(row.ticketId, {
          content: row.content ?? "",
    ...
    // Feed-identical hotScore ranking — same algorithm as /api/feed.
    // lastActivityAt = most recent of: createdAt, last like, last comment.
    // Highly-engaged older posts resurface ahead of brand-new zero-engagement ones,
    // matching the feed behaviour users already know.
    const scored = rawTickets.map((t) => {
      const likes    = likeMap.get(t.id) ?? 0;
      const comments = commentMap.get(t.id) ?? 0;
      const likeLastAt = likeLastAtMap.get(t.id);
      const cmtLastAt  = cmtLastAtMap.get(t.id);
      const lastActivityAt = new Date(Math.max(
        t.createdAt.getTime(),
        likeLastAt?.getTime() ?? 0,
        cmtLastAt?.getTime() ?? 0,
      ));
      return {
        ticket: t,
        score: hotScore({ likes, comments, lastActivityAt }),
      };
    });
    ...
    const result = topTickets.map((ticket) => {
      ...
      return {
        ...
        rating: ticket.hideRating ? null : (ticket.rating ? Number(ticket.rating) : null),
        hideRating: ticket.hideRating ?? false,
        ...
        likeCount: likeMap.get(ticket.id) ?? 0,
        commentCount: commentMap.get(ticket.id) ?? 0,
        hideLikes: ticket.hideLikes ?? false,
        hideComments: ticket.hideComments ?? false,
        topComment: (ticket.hideComments ?? false) ? null : (topCommentMap.get(ticket.id) ?? null),
      };
    });
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (isTv) {
        ...
        const showI = showInfo as any;
        const showGenreIds = showI.genre_ids ?? (showI.genres ?? []).map((g: any) => g.id);
        const isAnime = showI.original_language === "ja" && showGenreIds.includes(16);
        const showName = showI.name ?? "";
        ...
        if (isAnime && showName) {
          ...
          if (alRelations.length > 0) {
            ...
            const movies = relatedItems
              .filter(r => r.status === "fulfilled" && (r as any).value?.title)
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const seasonParam  = typeof req.query["season"]  === "string" ? parseInt(req.query["season"],  10) : NaN;
    const episodeParam = typeof req.query["episode"] === "string" ? parseInt(req.query["episode"], 10) : NaN;

    type TMDBImage = { file_path: string; vote_average?: number; vote_count?: number; iso_639_1?: string | null; width?: number; height?: number };

    const sortImages = (imgs: TMDBImage[]) =>
      [...imgs].sort((a, b) => {
        const aNeutral = (a.iso_639_1 == null) ? 0 : 1;
        const bNeutral = (b.iso_639_1 == null) ? 0 : 1;
    ...
    const images: ImageResult[] = [
      ...sortedPosters.map   (p => ({ url: `${TMDB_IMG}${p.file_path}`,      type: "poster"   as const })),
      ...sortedBackdrops.map (b => ({ url: `${TMDB_IMG_WIDE}${b.file_path}`, type: "backdrop" as const })),
    ];

    // Episode stills for TV shows when a specific episode is requested
    if (isTv && !isNaN(seasonParam) && !isNaN(episodeParam)) {
      try {
        const stillsData = await tmdbFetch<{ stills?: TMDBImage[] }>(
          `/tv/${tmdbId}/season/${seasonParam}/episode/${episodeParam}/images`,
        );
        const sortedStills = sortImages(stillsData.stills ?? []).slice(0, 20);
        images.push(...sortedStills.map(s => ({ url: `${TMDB_IMG_WIDE}${s.file_path}`, type: "still" as const })));
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const trailerCacheKey = `trailer:${isTv ? "tv" : "movie"}:${tmdbId}`;
    const cachedTrailer = await getCached(trailerCacheKey, TRAILER_TTL_MS);
    if (cachedTrailer) {
      return res.json(cachedTrailer);
    }
    ...
    setCached(trailerCacheKey, trailerResult).catch(() => {});
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    return res.json({ results, genreIds, keywordIds: usedKeywordIds, isTopicSearch });
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (results.length === 0) {
      ...
      if (!imdbId && !isTv) {
        const extIds = await tmdbFetch<{ imdb_id?: string | null }>(`/movie/${tmdbId}/external_ids`).catch(() => ({}) as { imdb_id?: string | null });
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (isTv) {
      ...
      const showD = showData as any;
      const genreIds = showD.genre_ids ?? (showD.genres ?? []).map((g: any) => g.id);
      isVoiceCast = (showD.original_language === "ja" && genreIds.includes(16));
      ...
      directors = ((showData as any).created_by ?? [])
        .slice(0, 5)
        .map((c: any) => ({
          id: c.id,
          name: c.name,
      ...
      const movieI = movieInfo as any;
      const genreIds = movieI.genre_ids ?? (movieI.genres ?? []).map((g: any) => g.id);
      isVoiceCast = (movieI.original_language === "ja" && genreIds.includes(16));
