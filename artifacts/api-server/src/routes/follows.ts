...
import { usersTable, followsTable, notificationsTable, followRequestsTable, userBlocksTable } from "@workspace/db/schema";
...
router.post("/:username/follow", followRateLimit, async (req, res) => {
  ...
  const [blockRow] = await db.select().from(userBlocksTable).where(
    or(
      and(eq(userBlocksTable.blockerId, currentUserId), eq(userBlocksTable.blockedId, target.id)),
      and(eq(userBlocksTable.blockerId, target.id), eq(userBlocksTable.blockedId, currentUserId)),
    )
  ).limit(1);
  if (blockRow) { res.status(403).json({ error: "blocked" }); return; }
...
// DELETE /api/users/:username/remove-follower — remove someone who follows
// *me* (the inverse of unfollow). currentUserId is the followingId, target
// is the followerId being removed.
router.delete("/:username/remove-follower", followRateLimit, async (req, res) => {
  const currentUserId = req.session?.userId;
  if (!currentUserId) { res.status(401).json({ error: "unauthorized" }); return; }
  const username = String(req.params["username"]);
  const [target] = await db.select().from(usersTable).where(eq(usersTable.username, username)).limit(1);
  if (!target) { res.status(404).json({ error: "not_found" }); return; }
  if (target.id === currentUserId) { res.status(400).json({ error: "bad_request" }); return; }

  await db.delete(followsTable).where(and(eq(followsTable.followerId, target.id), eq(followsTable.followingId, currentUserId)));
  await db.delete(followRequestsTable).where(and(eq(followRequestsTable.fromUserId, target.id), eq(followRequestsTable.toUserId, currentUserId)));
  emitFollowChanged({ followerId: target.id, followingId: currentUserId });
  res.json({ success: true });
});

// POST /api/users/:username/block — block a user.
// Removes any existing follow relationship in either direction (blocking
// implies unfollowing both ways) and any pending follow requests.
router.post("/:username/block", followRateLimit, async (req, res) => {
  const currentUserId = req.session?.userId;
  if (!currentUserId) { res.status(401).json({ error: "unauthorized" }); return; }
  const username = String(req.params["username"]);
  const [target] = await db.select().from(usersTable).where(eq(usersTable.username, username)).limit(1);
  if (!target) { res.status(404).json({ error: "not_found" }); return; }
  if (target.id === currentUserId) { res.status(400).json({ error: "bad_request", message: "Cannot block yourself" }); return; }

  const [existing] = await db.select().from(userBlocksTable).where(
    and(eq(userBlocksTable.blockerId, currentUserId), eq(userBlocksTable.blockedId, target.id))
  ).limit(1);
  if (!existing) {
    await db.insert(userBlocksTable).values({ blockerId: currentUserId, blockedId: target.id });
  }

  await db.delete(followsTable).where(
    or(
      and(eq(followsTable.followerId, currentUserId), eq(followsTable.followingId, target.id)),
      and(eq(followsTable.followerId, target.id), eq(followsTable.followingId, currentUserId)),
    )
  );
  await db.delete(followRequestsTable).where(
    or(
      and(eq(followRequestsTable.fromUserId, currentUserId), eq(followRequestsTable.toUserId, target.id)),
      and(eq(followRequestsTable.fromUserId, target.id), eq(followRequestsTable.toUserId, currentUserId)),
    )
  );
  emitFollowChanged({ followerId: currentUserId, followingId: target.id });
  emitFollowChanged({ followerId: target.id, followingId: currentUserId });
...
router.delete("/:username/block", followRateLimit, async (req, res) => {
  const currentUserId = req.session?.userId;
  if (!currentUserId) { res.status(401).json({ error: "unauthorized" }); return; }
  const username = String(req.params["username"]);
  const [target] = await db.select().from(usersTable).where(eq(usersTable.username, username)).limit(1);
