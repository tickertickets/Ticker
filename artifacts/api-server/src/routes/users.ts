...
router.get("/featured", async (req, res) => {
  const users = await db
    ...
    .from(usersTable)
    .innerJoin(followsTable, eq(followsTable.followingId, usersTable.id))
...
router.get("/:username/tickets", async (req, res) => {
  ...
  const rows = await (
    sortBy === "popular"
      ...
      : base.orderBy(
          sql`${ticketsTable.displayOrder} ASC NULLS FIRST`,
          desc(ticketsTable.createdAt),
        )
  ).limit(limit + 1);
...
router.get("/:username/followers", async (req, res) => {
  ...
  if (user.isPrivate && user.id !== currentUserId) {
    const isFollower = currentUserId ? (await db.select({ id: followsTable.followerId }).from(followsTable).where(
      and(eq(followsTable.followerId, currentUserId), eq(followsTable.followingId, user.id))
    ).limit(1)).length > 0 : false;
    if (!isFollower) {
...
// ── GET /users/me/activities — likes & comments history (last 30 days) ─────────
router.get("/me/activities", async (req, res) => {
  ...
  const [ticketLikes, ticketComments, chainLikesRows, chainCommentsRows, ownTicketRows] = await Promise.all([
    // Ticket likes
    db
      .select({
        id: sql<string>`${likesTable.ticketId}::text || '_tl'`,
      ...
      .from(likesTable)
      .innerJoin(ticketsTable, and(eq(ticketsTable.id, likesTable.ticketId), isNull(ticketsTable.deletedAt)))
      .where(and(eq(likesTable.userId, currentUserId), gte(likesTable.createdAt, thirtyDaysAgo)))
    ...
    db
      ...
      .from(commentsTable)
      .innerJoin(ticketsTable, and(eq(ticketsTable.id, commentsTable.ticketId), isNull(ticketsTable.deletedAt)))
      .where(and(eq(commentsTable.userId, currentUserId), gte(commentsTable.createdAt, thirtyDaysAgo)))
    ...
    db
      .select({
        id: sql<string>`${chainLikesTable.chainId}::text || '_cl'`,
        chainId: chainLikesTable.chainId,
        chainTitle: chainsTable.title,
        createdAt: chainLikesTable.createdAt,
      })
      .from(chainLikesTable)
      .innerJoin(chainsTable, and(eq(chainsTable.id, chainLikesTable.chainId), isNull(chainsTable.deletedAt)))
      .where(and(eq(chainLikesTable.userId, currentUserId), gte(chainLikesTable.createdAt, thirtyDaysAgo)))
      .orderBy(desc(chainLikesTable.createdAt))
      .limit(100),
    ...
    db
      ...
      .innerJoin(chainsTable, and(eq(chainsTable.id, chainCommentsTable.chainId), isNull(chainsTable.deletedAt)))
      .where(and(eq(chainCommentsTable.userId, currentUserId), gte(chainCommentsTable.createdAt, thirtyDaysAgo)))
    ...
    db
      .select({
        id: sql<string>`${ticketsTable.id}::text || '_ot'`,
        ticketId: sql<string>`${ticketsTable.id}::text`,
        movieTitle: ticketsTable.movieTitle,
        posterUrl: ticketsTable.posterUrl,
        rankTier: ticketsTable.rankTier,
      ...
      .where(and(
        ...
        gte(ticketsTable.createdAt, thirtyDaysAgo),
      ))
      .orderBy(desc(ticketsTable.createdAt))
      .limit(50),
  ]);

  // Batch-fetch first poster for each unique chain
  const chainIds = Array.from(new Set([
    ...chainLikesRows.map(r => r.chainId),
    ...chainCommentsRows.map(r => r.chainId),
  ]));
  let chainPosterMap = new Map<string, string | null>();
  if (chainIds.length > 0) {
    const posterRows = await db
      .select({ chainId: chainMoviesTable.chainId, posterUrl: chainMoviesTable.posterUrl })
      .from(chainMoviesTable)
      .where(inArray(chainMoviesTable.chainId, chainIds))
      .orderBy(asc(chainMoviesTable.position));
    // Keep only first poster per chain
    for (const row of posterRows) {
      if (!chainPosterMap.has(row.chainId)) {
        chainPosterMap.set(row.chainId, row.posterUrl ?? null);
      }
    }
  }
  ...
  res.json({
    ticketLikes,
    ticketComments,
    chainLikes: chainLikesRows.map(r => ({ ...r, posterUrl: chainPosterMap.get(r.chainId) ?? null })),
