...
function buildRules() {
  return {
    xpRequired: XP_PER_LEVEL,
    // dailyCap has been removed — no hard daily limits.
    // Anti-spam is enforced via unique sourceId per event (see badge.service.ts comments).
    sources: [
      {
        action: "post_ticket",
        label: "โพสต์ Ticket",
        xpPerAction: XP_PER_ACTION.post_ticket,
        note: "ต่อโพสต์ • ไม่จำกัดครั้ง",
      },
      {
        action: "post_chain",
        label: "โพสต์ Chain",
      ...
      {
        action: "tag_ticket",
        label: "Tag เพื่อนใน Ticket",
        xpPerAction: XP_PER_ACTION.tag_ticket,
        note: "ต่อ Ticket • สูงสุด 2 ครั้ง/วัน",
      },
      {
        action: "party_complete",
        label: "Party ครบทุกคน",
        xpPerAction: XP_PER_ACTION.party_complete,
        note: "ทุกคนยอมรับ • สูงสุด 2 ครั้ง/วัน (+1 XP/คน ถ้า Tag ≥3 คน)",
      },
    ],
  };
}
