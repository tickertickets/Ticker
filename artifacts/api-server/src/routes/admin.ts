...
/**
 * POST /api/admin/chains/:id/clear-cover
 * Clears the tagged movie cover image from a Chain (removes movie tag association).
 * Used to process DMCA / copyright takedown requests for chains. Admin only.
 */
router.post("/chains/:id/clear-cover", async (req, res) => {
  const adminId = req.session?.userId;
  if (!isAdmin(adminId)) { res.status(403).json({ error: "forbidden" }); return; }
  const { id } = req.params;
  const rows = await db.select({ id: chainsTable.id, title: chainsTable.title, userId: chainsTable.userId })
    .from(chainsTable).where(eq(chainsTable.id, id)).limit(1);
  if (!rows[0]) { res.status(404).json({ error: "not_found" }); return; }
  await db.update(chainsTable)
    .set({ taggedMovieImdbId: null, taggedMovieTitle: null, taggedMoviePosterUrl: null })
    .where(eq(chainsTable.id, id));
  await sendDiscordWebhook("", [{
    title: "🖼️ Admin ลบภาพ Chain (DMCA Takedown)",
    color: 0xFFA500,
    fields: [
      { name: "Chain ID", value: id, inline: true },
      { name: "ชื่อ Chain", value: rows[0].title ?? "?", inline: true },
