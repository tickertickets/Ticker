...
router.get("/", async (req, res) => {
  ...
  const notifications = await db.select({
    ...
    // leftJoin instead of innerJoin: if the sender's account was deleted their
    // notifications must still appear (fromUser fields will be null, rendered
    // as "Deleted user").  innerJoin silently drops every such row from the
    // list — the in-app notification is present in the DB but invisible.
  ...
  res.json({
    notifications: items.map(n => ({
      ...
      fromUser: n.fromUser
        ...
        : { id: n.notif.fromUserId, username: "deleted_user", displayName: null, avatarUrl: null },
