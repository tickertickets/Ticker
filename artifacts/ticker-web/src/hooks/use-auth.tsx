...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const logout = async () => {
    // ─── Step 1: Synchronous — do this before any await ───────────────────
    // Flip loggingOut so user = null immediately in every consumer.
    setLoggingOut(true);
    setCachedUser(null);
    setAuthError(null);

    // Clear every local store now (synchronous).
    localStorage.removeItem(USER_CACHE_KEY);
    try { localStorage.removeItem("TICKER_RQ_v1"); } catch { /* ignore */ }
    sessionStorage.clear();
    scrollStore.clear();
    clearAccountState();
    resetTheme();

    // Navigate away from the current private route RIGHT NOW, before any
    // async work. Without this the user stays on /settings while we await.
    window.history.replaceState(null, "", import.meta.env.BASE_URL ?? "/");
    setLocation("/");

    // ─── Step 2: Async — run push cleanup + server logout in parallel ──────
    // Wrap disablePushNotifications in a 3-second timeout: it calls
    // navigator.serviceWorker.ready internally which can hang indefinitely
    // if the SW is still installing.
    const withTimeout = <T,>(p: Promise<T>, ms: number): Promise<T | void> =>
      Promise.race([p, new Promise<void>((res) => setTimeout(res, ms))]);

    await Promise.allSettled([
      withTimeout(disablePushNotifications(), 3000),
      fetch("/api/auth/logout", { credentials: "include", method: "POST" }),
    ]);

    // ─── Step 3: Hard reload ───────────────────────────────────────────────
    // Full page reload clears every in-memory cache so a subsequent load
    // always starts from a clean slate, regardless of what the server
    // session state ended up being.
  ...
  const loginWithEmail = async (email: string, password: string) => {
    try {
      setAuthError(null);
      const response = await fetch("/api/auth/supabase/login", { credentials: "include",
        method: "POST",
  ...
  const signupWithEmail = async (email: string, password: string) => {
    try {
      setAuthError(null);
      const response = await fetch("/api/auth/supabase/signup", { credentials: "include",
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, password }),
      });
