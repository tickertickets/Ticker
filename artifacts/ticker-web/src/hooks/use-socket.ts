...
export function useSocketFeedUpdates() {
  ...
  useEffect(() => {
    ...
    const handleFeedNew = () => {
      ...
      // Signal the feed page to do a full reset (scroll to top + clear extra pages)
      // so the new post appears at the top immediately instead of requiring manual pull-refresh.
      window.dispatchEvent(new CustomEvent("ticker:feed-socket-new"));
    };
...
/**
 * Listens for badge:update events and invalidates the badge query cache
 * immediately so XP / level changes reflect in real time without waiting
 * for the 60s poll interval.
 */
export function useSocketBadgeUpdates() {
  const qc = useQueryClient();

  useEffect(() => {
    const socket = getSocket();

    const handleBadgeUpdate = (data?: { userId?: string }) => {
      // Invalidate own badge in settings
      qc.invalidateQueries({ queryKey: ["badge-me"] });
      // Also invalidate the user-specific badge shown on profiles / feed cards
      if (data?.userId) {
        qc.invalidateQueries({ queryKey: ["badge-user", data.userId] });
      }
    };

    socket.on("badge:update", handleBadgeUpdate);
    return () => { socket.off("badge:update", handleBadgeUpdate); };
  }, [qc]);
}
...
export function useSocketChainUpdates(chainId: string | undefined) {
  ...
  useEffect(() => {
    ...
    // ── chain comment events — keep comment list in sync for other users ──
    const handleCommentNew = () => {
      qc.invalidateQueries({ queryKey: ["/api/chains", chainId, "comments"] });
      qc.invalidateQueries({ queryKey: [`chain-comments-${chainId}`] });
    ...
    socket.on("chain:comment:new", handleCommentNew);
    socket.on("chain:comment:deleted", handleCommentDeleted);
    socket.on("chain:comment:updated", handleCommentUpdated);

    return () => {
      socket.emit("chain:leave", chainId);
      socket.off("chain:updated", handleChainUpdated);
      socket.off("chain:comment:new", handleCommentNew);
      socket.off("chain:comment:deleted", handleCommentDeleted);
      socket.off("chain:comment:updated", handleCommentUpdated);
    };
  }, [chainId, qc]);
}
...
export function useSocketTicketUpdates(ticketId: string | undefined) {
  ...
  useEffect(() => {
    ...
    // ── comment:updated — refetch comment list (content changed) ──────────
    const handleCommentUpdated = () => {
      qc.invalidateQueries({ queryKey: [`/api/tickets/${ticketId}/comments`] });
    };
