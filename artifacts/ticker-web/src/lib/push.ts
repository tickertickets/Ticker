...
export async function getSwRegistration(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Fast path: if a SW is already active (returning user), getRegistration()
    // resolves immediately without waiting for precaching to complete.
    const existing = await navigator.serviceWorker.getRegistration("/");
    if (existing?.active) return existing;
    // Slow path: SW is still installing (new user — workbox is precaching all
    // app assets before activation). Wait until it finishes and activates.
    // navigator.serviceWorker.ready never rejects; it resolves once the SW
    // reaches the "activating" or "activated" state.
...
export async function enablePushNotifications(): Promise<{ ok: boolean; reason?: string }> {
  ...
  try {
    // 30 s — generous budget for the SW install step on slow mobile connections.
    // With the reduced precache (index.html + icons only) this should resolve
    // in < 1 s, but the larger budget protects against unexpected delays.
    reg = await withTimeout(getSwRegistration(), 30000, "sw_ready");
  } catch {
