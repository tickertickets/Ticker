...
export default function AuthSignup() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Navigate immediately — the session cookie is already set by the server.
      // refreshUser() is called in the background so AuthContext warms up while
      // onboarding is mounting (avoids blocking navigation on a round-trip that
      // can fail on cold-start Render servers and cause a false red error).
      // Onboarding shows a spinner until `user` resolves, so there is no flash.
      void refreshUser();
      navigate("/onboarding");
      return;
    } catch {
