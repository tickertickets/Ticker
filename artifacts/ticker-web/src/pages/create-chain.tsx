...
export default function CreateChain() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    setSubmitError("");
    // Track whether the chain was created on the server — cleanup errors after
    // this point should not show a false "save failed" error (issue 1.5.1).
    ...
    try {
      ...
      chainCreated = true; // server confirmed creation — cleanup below is non-fatal
      ...
      if (chainCreated) {
        // Chain was created but a cleanup step failed — navigate home so user
        // can see their post rather than being stuck on the create page.
        setLocation("/");
        return;
      }
        ...
        <div className="fixed inset-0 z-[9999] flex items-end" onClick={() => setShowDraftDialog(false)}>
          ...
          <div
            ...
            style={{
              ...
              paddingBottom: "max(1.5rem, var(--sai-bottom) + 1rem)",
            }}
            onClick={e => e.stopPropagation()}
          >
