...
function FilmCard({ film, navSrclang }: { film: CharFilm; navSrclang: string }) {
  ...
  return (
    <Link href={href} onClick={() => { if (film.imdbId) { scrollStore.delete(`movie-${film.imdbId}`); scrollStore.delete(`movie-${film.imdbId}-details`); } }}>
      ...
      >
        <PosterImage src={film.posterUrl} alt={film.title} fallbackIcon={<Film className="w-5 h-5 text-zinc-500" />} />
...
export default function CharacterDetail() {
  ...
  const { data, isLoading, isError, isPending } = useQuery<CharData>({
    queryKey: ["/api/character", wikidataId],
    queryFn: async () => {
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), 20_000);
      try {
        const res = await fetch(`/api/character/${encodeURIComponent(wikidataId)}`, { credentials: "include", signal: ctrl.signal });
  ...
  const { data: bookmarkData } = useQuery<{ isBookmarked: boolean }>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/character/${encodeURIComponent(bookmarkCharId)}/bookmark`, { credentials: "include" });
  ...
  const bookmarkMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/character/${encodeURIComponent(bookmarkCharId)}/bookmark`, { credentials: "include", method: "POST" });
  ...
  const submitReport = useCallback(async () => {
    ...
    setSubmitting(true);
    try {
      await fetch(`/api/reports/character/${encodeURIComponent(data?.charId ?? wikidataId)}`, { credentials: "include",
        method: "POST",
      ...
      {showReport && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50" onClick={closeReport}>
          <div
            ...
            style={{ paddingBottom: "max(1.5rem, var(--sai-bottom))" }}
            onClick={(e) => e.stopPropagation()}
          >
