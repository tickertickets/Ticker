...
import { navBack, clearMovieRestore } from "@/lib/nav-back";
...
function PersonMovieCard({ movie, navSrclang }: { movie: PersonMovie; navSrclang: string }) {
  ...
  // Prefer cached movie detail (more accurate data) over the person-API snapshot
  const cached = qc.getQueryData<any>(["/api/movies", movie.imdbId]);
  const input: ScoreInput = {
    tmdbRating: cached ? parseFloat(cached.tmdbRating ?? cached.imdbRating ?? "0") : parseFloat(movie.tmdbRating ?? "0"),
    voteCount: cached?.voteCount ?? movie.voteCount ?? 0,
    genreIds: cached?.genreIds ?? movie.genreIds ?? [],
    popularity: cached?.popularity ?? movie.popularity ?? 0,
    franchiseIds: cached?.franchiseIds ?? movie.franchiseIds ?? [],
    year: movie.year ? parseInt(movie.year) : undefined,
    releaseDate: cached?.releaseDate ?? movie.releaseDate ?? null,
  };
  ...
  return (
    <Link href={href} onClick={() => { scrollStore.delete(`movie-${movie.imdbId}`); scrollStore.delete(`movie-${movie.imdbId}-details`); clearMovieRestore(movie.imdbId); }}>
      <div
        className="relative rounded-xl overflow-hidden bg-zinc-900 border border-border shimmer-no-border w-full"
        style={{ aspectRatio: "2/3" }}
      >
        <PosterImage src={movie.posterUrl} alt={movie.title} fallbackIcon={<Film className="w-5 h-5 text-zinc-500" />} />
...
export default function PersonDetail() {
  ...
  const { data, isLoading, isError, isPending } = useQuery<PersonData>({
    queryKey: ["/api/person", personId, apiLang],
    queryFn: async () => {
      const res = await fetch(`/api/person/${encodeURIComponent(personId)}?lang=${apiLang}`, { credentials: "include" });
  ...
  const { data: awardsData } = useQuery<{ results: AwardResult[] }>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/person/${encodeURIComponent(personId)}/awards`, { credentials: "include" });
      ...
      <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto overscroll-y-none">
        ...
        {data && (
          <div>
            ...
            <div style={{ height: "calc(var(--sai-bottom) + 0.75rem)" }} aria-hidden />
          </div>
        )}
      </div>
    </div>
