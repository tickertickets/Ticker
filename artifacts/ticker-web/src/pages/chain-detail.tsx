...
export default function ChainDetail() {
  ...
  const [editingChainCommentId, setEditingChainCommentId] = useState<string | null>(null);
  const [editChainCommentContent, setEditChainCommentContent] = useState("");
  ...
  const { data: chain, isLoading, isPending, isError: isChainError } = useQuery<ChainData>({
    ...
    queryFn: async () => {
      ...
      // 404 → chain ถูกลบหรือไม่พบจริง; errors อื่นให้ throw เป็น transient error
      if (res.status === 404) return null as unknown as ChainData;
  ...
  useEffect(() => {
    if (!chain) return;
    const title = chain.title ? `${chain.title} | Ticker` : "Ticker";
    const desc = chain.description
      ? chain.description.slice(0, 150) + (chain.description.length > 150 ? "…" : "")
      : `${chain.user?.displayName ?? chain.user?.username ?? ""}${chain.user ? " · " : ""}Chain on Ticker`;
    const imageUrl = chain.movies?.[0]?.posterUrl ?? null;
    const pageUrl = `https://ticker-tickets.vercel.app/chain/${chainId}`;
    ...
    if (imageUrl) setMeta("twitter:image", imageUrl, true);
    return () => {
  ...
  const deleteChainComment = async (commentId: string) => {
    // Optimistic: ลบออกก่อน แล้วค่อย API
    qc.setQueryData<{ comments: ChainComment[] }>(
      ["/api/chains", chainId, "comments"],
      (old) => old ? { comments: old.comments.filter(c => c.id !== commentId) } : old
    );
    qc.invalidateQueries({ queryKey: [`chain-comments-preview-${chainId}`] });
    try {
      const res = await fetch(`/api/chains/${chainId}/comments/${commentId}`, { method: "DELETE", credentials: "include" });
      if (!res.ok) {
        // ถ้าล้มเหลวให้โหลดใหม่ ไม่ navigate ออก
        refetchChainComments();
        toast({ title: "ไม่สามารถลบความคิดเห็นได้", variant: "destructive" });
      }
    } catch {
      refetchChainComments();
    }
  ...
  const editChainComment = async (commentId: string) => {
    if (!editChainCommentContent.trim()) return;
    try {
      const res = await fetch(`/api/chains/${chainId}/comments/${commentId}`, {
        method: "PATCH",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ content: editChainCommentContent.trim() }),
      });
      if (res.ok) {
        qc.setQueryData<{ comments: ChainComment[] }>(
          ["/api/chains", chainId, "comments"],
          (old) => old ? { comments: old.comments.map(c => c.id === commentId ? { ...c, content: editChainCommentContent.trim() } : c) } : old
        );
        qc.invalidateQueries({ queryKey: [`chain-comments-preview-${chainId}`] });
        setEditingChainCommentId(null);
        setEditChainCommentContent("");
      } else {
        toast({ title: "ไม่สามารถแก้ไขความคิดเห็นได้", variant: "destructive" });
      }
    } catch { /* non-fatal */ }
  };
  ...
  if (!chain) {
    ...
    if (isChainError) return (
      <div className="h-full bg-background flex flex-col items-center justify-center gap-3">
        <p className="text-muted-foreground">เกิดข้อผิดพลาด โปรดลองใหม่อีกครั้ง</p>
        <button
          onClick={() => qc.invalidateQueries({ queryKey: ["/api/chains", chainId] })}
          className="text-sm underline"
        >
          ลองใหม่
        </button>
        <button onClick={() => navBack(navigate)} className="text-xs text-muted-foreground underline">ย้อนกลับ</button>
      </div>
    );
    return (
      <div className="h-full bg-background flex flex-col items-center justify-center gap-3">
                  ...
                  {user?.id === c.userId && (
                    confirmDeleteCommentId === c.id ? (
                      ...
                      <div className="ml-auto flex items-center gap-0.5">
                        {editingChainCommentId !== c.id && (
                          <button
                            onClick={() => { setEditChainCommentContent(c.content); setEditingChainCommentId(c.id); setConfirmDeleteCommentId(null); }}
                            className="p-1 text-muted-foreground/40 hover:text-foreground transition-colors"
                          >
                            <Pencil className="w-3 h-3" />
                          </button>
                        )}
                        <button onClick={() => setConfirmDeleteCommentId(c.id)} className="p-1 text-muted-foreground/40 hover:text-destructive transition-colors">
                ...
                {editingChainCommentId === c.id && (
                  <div className="mt-2 flex gap-2">
                    <textarea
                      ...
                      value={editChainCommentContent}
                      onChange={(e) => setEditChainCommentContent(e.target.value)}
                      rows={2}
                      autoFocus
                      maxLength={500}
                    />
                    <div className="flex flex-col gap-1 shrink-0">
                      <button onClick={() => editChainComment(c.id)} className="px-3 py-1.5 bg-foreground text-background text-[10px] font-bold rounded-xl">บันทึก</button>
                      <button onClick={() => { setEditingChainCommentId(null); setEditChainCommentContent(""); }} className="px-3 py-1.5 bg-secondary text-muted-foreground text-[10px] rounded-xl">ยกเลิก</button>
                    </div>
                  </div>
                )}
                {!isReply && user && (
