...
import { ArrowLeft, CheckCircle, XCircle, Trash2, ExternalLink, Clock, Megaphone, Popcorn, Flag, Users, Search, Send } from "lucide-react";
import { Loader2 } from "lucide-react";
import { navBack } from "@/lib/nav-back";
import { useAuth } from "@/hooks/use-auth";

type FilterStatus = "pending" | "approved" | "rejected" | "all";
type Tab = "users" | "verify" | "broadcast" | "reports";
...
const REASON_TH: Record<string, string> = {
  ...
  copyright_infringement: "ละเมิดลิขสิทธิ์ (DMCA)",
  other: "อื่นๆ",
};
...
function ReportsPanel() {
  ...
  const clearImage = useMutation({
    mutationFn: (id: string) => fetch(`/api/admin/tickets/${id}/clear-image`, { method: "POST", credentials: "include" }).then(r => { if (!r.ok) throw new Error("failed"); return r.json(); }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["admin-reports"] }),
  });
  const clearChainCover = useMutation({
    mutationFn: (id: string) => fetch(`/api/admin/chains/${id}/clear-cover`, { method: "POST", credentials: "include" }).then(r => { if (!r.ok) throw new Error("failed"); return r.json(); }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["admin-reports"] }),
  });
          ...
          {reports.map(r => (
            <div key={r.id} className="px-4 py-3 space-y-2">
              ...
              <div className="flex gap-1.5 flex-wrap">
                ...
                {r.reason === "copyright_infringement" && r.ticketId && (
                  <button
                    onClick={() => { if (confirm("ลบภาพออกจาก Ticket นี้? (DMCA Takedown)\nTicket จะยังอยู่แต่ภาพจะถูกนำออก")) clearImage.mutate(r.ticketId!); }}
                    disabled={clearImage.isPending}
                    className="flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-bold text-orange-400 bg-orange-500/10 hover:bg-orange-500/20 active:scale-95 transition-all disabled:opacity-40"
                  >
                    {clearImage.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : <span>🖼️</span>}
                    ลบภาพ Ticket (DMCA)
                  </button>
                )}
                {r.reason === "copyright_infringement" && r.chainId && (
                  <button
                    onClick={() => { if (confirm("ลบภาพหน้าปก Chain นี้? (DMCA Takedown)\nChain จะยังอยู่แต่ภาพ cover จะถูกนำออก")) clearChainCover.mutate(r.chainId!); }}
                    disabled={clearChainCover.isPending}
                    className="flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-bold text-orange-400 bg-orange-500/10 hover:bg-orange-500/20 active:scale-95 transition-all disabled:opacity-40"
                  >
                    {clearChainCover.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : <span>🖼️</span>}
                    ลบภาพ Chain (DMCA)
                  </button>
                )}
              </div>
...
export default function AdminPanel() {
  ...
  return (
    <div className="h-full bg-background flex flex-col">
      ...
      {/* Section tabs */}
      <div className="flex gap-1 px-4 py-3 border-b border-border overflow-x-auto">
        {([["users", <Users className="w-3.5 h-3.5" />, "ผู้ใช้"], ["verify", <Popcorn className="w-3.5 h-3.5" />, "Verify"], ["broadcast", <Megaphone className="w-3.5 h-3.5" />, "ประกาศ"], ["reports", <Flag className="w-3.5 h-3.5" />, "รายงาน"]] as [Tab, ReactNode, string][]).map(([id, icon, label]) => (
