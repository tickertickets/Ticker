...
import { Loader2, ChevronLeft, Send, MapPin, CalendarDays, Trash2, MessagesSquare, Flag, Share2, EyeOff, CornerDownRight } from "lucide-react";
          ...
          <div className="px-4">
            {commentCount === 0 && localComments.filter(c => c.isPending).length === 0 ? (
              ...
              const renderComment = (comment: LocalComment, isReply = false) => {
                ...
                return (
                  <div key={comment.id} className={cn("flex gap-3", comment.isPending && "opacity-60", isReply && "ml-8 pl-3 border-l border-border")}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1 mb-1 flex-wrap">
                        ...
                        {!comment.isPending && user && (user.id === comment.userId || isOwner) && (
                          <button onClick={() => handleDeleteComment(comment.id)} className="ml-auto p-1 text-muted-foreground/40 hover:text-destructive transition-colors">
                      ...
                      <div className="bg-secondary rounded-2xl rounded-tl-sm px-3.5 py-2.5">
                        <p className="text-sm text-foreground/90 leading-relaxed break-words" style={{ overflowWrap: "break-word", wordBreak: "break-word" }}>{comment.content}</p>
                      </div>
