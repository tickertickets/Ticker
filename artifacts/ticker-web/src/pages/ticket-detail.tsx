...
import { Loader2, ChevronLeft, Send, MapPin, CalendarDays, Trash2, MessagesSquare, Flag, Share2, EyeOff, CornerDownRight, Pencil } from "lucide-react";
...
export default function TicketDetail() {
  ...
  // Scroll to specific comment when navigating from notification
  useEffect(() => {
    const commentId = new URLSearchParams(window.location.search).get("commentId");
    if (!commentId || localComments.length === 0) return;
  ...
  const { data: ticket, isLoading: ticketLoading, isPending: ticketPending } = useGetTicket(ticketId || "", {
  ...
  useEffect(() => {
    if (commentsData?.comments) {
      type RawComment = (typeof commentsData.comments)[number];
      const serverComments: LocalComment[] = commentsData.comments.map((c: RawComment) => ({
        id: c.id,
        userId: c.userId,
        content: c.content,
        replyToId: (c as any).replyToId ?? null,
  ...
  useEffect(() => {
    ...
    if (movieTitle) {
      ...
      const pageUrl = `https://ticker-tickets.vercel.app/ticket/${ticketId}`;
  ...
  const handleComment = async (e: React.FormEvent) => {
    ...
    try {
      await createComment.mutateAsync({ ticketId, data: { content: text, replyToId: replyId } as any });
  ...
  const handleEditComment = async (commentId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setLocalComments((prev) => prev.map((c) => c.id === commentId ? { ...c, content: editContent.trim() } : c));
        queryClient.invalidateQueries({ queryKey: [`/api/tickets/${ticketId}/comments`] });
        setEditingCommentId(null);
        setEditContent("");
      } else {
        const body = await res.json().catch(() => ({}));
        toast({ title: "ไม่สามารถแก้ไขความคิดเห็นได้", description: body?.error ?? "ลองใหม่อีกครั้ง", variant: "destructive" });
      }
    } catch {
  ...
  const confirmDeleteTicket = async () => {
    ...
    try {
      ...
      // Optimistically remove from every list/feed cache immediately so the
      // feed reflects the deletion the moment the user navigates back.
      queryClient.setQueriesData(
        { queryKey: ["/api/tickets"] },
        (old: any) => {
          if (!old?.tickets) return old;
          return { ...old, tickets: old.tickets.filter((t: any) => t.id !== ticketId) };
        },
      );
      queryClient.setQueryData(["mixed-feed"], (old: any) => {
        if (!old?.items) return old;
        return { ...old, items: old.items.filter((item: any) => item.ticket?.id !== ticketId) };
      });
      queryClient.invalidateQueries({ queryKey: ["/api/tickets"] });
      queryClient.invalidateQueries({ queryKey: ["mixed-feed"] });
      if (ticket.user?.username) {
  ...
  if (!ticket) {
    if (ticketLoading || ticketPending) return (
    ...
    <div className="flex flex-col overflow-hidden h-full">
      ...
      <div className="z-30 bg-background border-b border-border px-4 py-4 flex items-center gap-3 flex-shrink-0 relative">
        ...
        <div className="flex items-center gap-1 ml-auto">
          ...
          {isOwner ? (
            ...
            // Reporting is allowed on every account's posts, including the
            // official Tickerofficial account — verification only affects the
            // badge shown next to the name, not moderation access.
            <button
      ...
      <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto overscroll-y-none">
        ...
        <div className="px-4 pt-2 pb-2">
          ...
          <div className="flex gap-2 mt-3 flex-wrap">
            ...
            {(ticket as any).createdAt && (
              <span className="flex items-center gap-1 px-3 py-1 bg-secondary rounded-full text-xs font-medium text-muted-foreground">
                <span className="text-[10px] font-bold opacity-60">{lang === "th" ? "โพสต์" : "Posted"}</span>
                {displayDate((ticket as any).createdAt, lang, { month: "short", day: "numeric", year: "numeric" })}
              </span>
            )}
            {ticket.location && (
              <span className="flex items-center gap-1 px-3 py-1 bg-secondary rounded-full text-xs font-medium text-muted-foreground">
        ...
        {(episodeLabel || caption) && (
          <div className="mx-4 mt-4 space-y-1">
            ...
            {caption && (() => {
              ...
              const alignClass = captionAlign === "center" ? "text-center" : captionAlign === "right" ? "text-right" : "text-left";
              return (((ticket as unknown) as Record<string, unknown>)["isSpoiler"] === true) && !captionRevealed ? (
                <button
                  className="flex items-start gap-2 text-left w-full"
                  onClick={() => setCaptionRevealed(true)}
                >
                ...
                <ExpandableText
                  text={caption}
                  className={`text-sm text-foreground leading-relaxed ${alignClass}`}
                />
              );
            })()}
          </div>
        ...
        {isOwner && linkSheetOpen && ticketId && (
          <AddLinkSheet
            ...
            onSaved={(newLinks) => {
              ...
              syncTicketLinks(ticketId, newLinks);
            }}
          />
        )}
        ...
        <div className="mt-5 border-t border-border">
          ...
          {/* Comments list */}
          <div className="px-4">
            {commentCount === 0 && localComments.filter(c => c.isPending).length === 0 ? (
              ...
              const rootComments = localComments.filter(c => !c.replyToId);
              const repliesMap = new Map<string, LocalComment[]>();
              ...
              const renderComment = (comment: LocalComment, isReply = false) => {
                ...
                if (comment.createdAt) {
                  ...
                  dateStr = mins < 1 ? t.timeJustNow
                    : mins < 60 ? `${mins} ${t.timeMin}`
                    : hours < 24 ? `${hours} ${t.timeHr}`
                    : days < 7 ? `${days} ${t.timeDay}`
                    : displayDate(comment.createdAt, lang, { month: "short", day: "numeric" });
                }
                return (
                  <div id={`comment-${comment.id}`} key={comment.id} className={cn("flex gap-3", comment.isPending && "opacity-60", isReply && "ml-8 pl-3 border-l border-border")}>
                    <Link href={`/profile/${comment.user?.username}`}>
                      <div className={cn("rounded-lg overflow-hidden bg-black border border-white/10 flex-shrink-0 flex items-center justify-center", isReply ? "w-7 h-7" : "w-8 h-8")}>
                        {comment.user?.avatarUrl ? (
                          ...
                          <span className="text-xs font-bold text-white">{comment.user?.displayName?.[0]?.toUpperCase() ?? "T"}</span>
                        )}
                      </div>
                    </Link>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1 mb-1 flex-wrap">
                        <Link href={`/profile/${comment.user?.username}`}>
                          <span className="text-xs font-bold text-foreground">{comment.user?.displayName ?? comment.user?.username}</span>
                        </Link>
                        {isVerified(comment.user?.username) && <VerifiedBadge className="w-3 h-3 flex-shrink-0" />}
                        ...
                        {!comment.isPending && user && (user.id === comment.userId || isOwner) && (
                          confirmDeleteCommentId === comment.id ? (
                            <div className="ml-auto flex items-center gap-1">
                              ...
                              <button onClick={() => { handleDeleteComment(comment.id); setConfirmDeleteCommentId(null); }} className="text-[10px] font-bold text-destructive px-1.5 py-0.5 rounded-lg bg-destructive/10">ลบ</button>
                            </div>
                          ) : (
                            <div className="ml-auto flex items-center gap-0.5">
                              {user.id === comment.userId && editingCommentId !== comment.id && (
                                <button onClick={() => { setEditContent(comment.content); setEditingCommentId(comment.id); }} className="p-1 text-muted-foreground/40 hover:text-foreground transition-colors">
                                  <Pencil className="w-3 h-3" />
                                </button>
                              )}
                              <button onClick={() => setConfirmDeleteCommentId(comment.id)} className="p-1 text-muted-foreground/40 hover:text-destructive transition-colors">
                        ...
                        {!comment.isPending && user && user.id !== comment.userId && !isOwner && (
                          <button onClick={() => setReportCommentId(comment.id)} className="ml-auto p-1 text-muted-foreground/40 hover:text-muted-foreground transition-colors">
                            <Flag className="w-3 h-3" />
                          </button>
                        )}
                      </div>
                      <div className="bg-secondary rounded-2xl rounded-tl-sm px-3.5 py-2.5">
                        <p className="text-sm text-foreground/90 leading-relaxed break-words" style={{ overflowWrap: "break-word", wordBreak: "break-word" }}>{renderCommentContent(comment.content)}</p>
                      </div>
                      {editingCommentId === comment.id && (
                        <div className="mt-2 flex gap-2">
                          ...
                          <div className="flex flex-col gap-1 shrink-0">
                            <button onClick={() => handleEditComment(comment.id)} className="px-3 py-1.5 bg-foreground text-background text-[10px] font-bold rounded-xl">บันทึก</button>
                      ...
                      {!isReply && !comment.isPending && user && (
                        <button
                          onClick={() => setReplyingTo({ id: comment.id, username: comment.user?.username ?? comment.user?.displayName ?? "?" })}
              ...
              return (
                <div className="overflow-y-auto max-h-[400px] space-y-4 mt-1 pb-4">
        ...
        >
          {replyingTo && (
            <div className="flex items-center gap-1.5 mb-2 px-1">
              ...
              <span className="text-xs text-muted-foreground">ตอบ @{replyingTo.username}</span>
