  ...
  return (
    <div className="flex items-center gap-3 bg-secondary rounded-2xl p-3 border border-border">
      <div className="relative w-10 h-14 rounded-xl overflow-hidden bg-zinc-900 flex-shrink-0 border border-border">
        <PosterImage src={ticket.posterUrl} alt={ticket.movieTitle} fallbackIcon={<Film className="w-4 h-4 text-muted-foreground" />} />
      </div>
...
function ChainTrashItem({ chain, onRestore, onPurge }: {
  ...
  return (
    <div className="flex items-center gap-3 bg-secondary rounded-2xl p-3 border border-border">
      <div className="relative w-10 h-14 rounded-xl overflow-hidden bg-zinc-900 flex-shrink-0 border border-border">
        <PosterImage src={poster} alt={chain.title} fallbackIcon={<Link2 className="w-4 h-4 text-muted-foreground" />} />
      </div>
...
type BadgeData = {
  ...
  rules: {
    xpRequired: number;
    sources: { action: string; label: string; xpPerAction: number; note: string }[];
  };
} | null;
...
function BadgeSection({
  ...
  onRegisterAdvance?: (fn: (newLevel: number) => void) => void;
}) {
  const { t } = useLang();
  ...
  // Register the carousel-advance callback so the parent can call it after evolve succeeds
  useEffect(() => {
    if (onRegisterAdvance) {
      onRegisterAdvance((newLevel: number) => {
        setActiveDot(newLevel);
      });
    }
                        ...
                        <div className="space-y-1.5">
                          ...
                          {canEvolve ? (
                            ...
                            <>
                              ...
                              {showXpHint && (
                                <div className="rounded-xl p-3 space-y-2" style={{ background: `${col}12` }}>
                                  {/* Header */}
                                  <p className="text-[10px] font-bold text-center" style={{ color: col }}>{t.howToEarnXP}</p>

                                  {/* Post row */}
                                  <div className="rounded-lg px-2.5 py-2 space-y-0.5" style={{ background: "rgba(59,130,246,0.10)" }}>
                                    <div className="flex justify-between items-center">
                                      <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                                        <TrendingUp className="w-3 h-3 text-blue-400" />
                                        Ticket / Chain
                                      </span>
                                      <span className="text-[11px] font-extrabold text-blue-400">{t.xpPerPost}</span>
                                    </div>
                                    <p className="text-[9px] text-muted-foreground/70 text-right">{t.xpCapPost}</p>
                                  </div>

                                  {/* Tag row */}
                                  <div className="rounded-lg px-2.5 py-2 space-y-0.5" style={{ background: "rgba(168,85,247,0.10)" }}>
                                    <div className="flex justify-between items-center">
                                      <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                                        <Users className="w-3 h-3 text-purple-400" />
                                        Tag เพื่อน
                                      </span>
                                      <span className="text-[11px] font-extrabold text-purple-400">{t.xpPerTag}</span>
                                    </div>
                                    <p className="text-[9px] text-muted-foreground/70 text-right">{t.xpCapTag}</p>
                                  </div>

                                  {/* Party row */}
                                  <div className="rounded-lg px-2.5 py-2 space-y-0.5" style={{ background: "rgba(245,158,11,0.10)" }}>
                                    <div className="flex justify-between items-center">
                                      <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                                        <Star className="w-3 h-3 text-amber-400" />
                                        {t.xpParty}
                                      </span>
                                      <span className="text-[11px] font-extrabold text-amber-400">{t.xpPerParty}</span>
                                    </div>
                                    <p className="text-[9px] text-muted-foreground/70 text-right">{t.xpCapParty}</p>
                                    <p className="text-[9px] text-amber-400/60 text-right">{t.xpBonusParty}</p>
                                  </div>

                                  {/* Goal line */}
                                  <div className="flex items-center justify-between pt-0.5 border-t border-border/30">
                                    <span className="text-[9px] text-muted-foreground/60">เป้าหมาย</span>
                                    <span className="text-[9px] font-bold" style={{ color: col }}>{xpRequired} XP → EVO</span>
                                  </div>
                                </div>
                              )}
                            </>
...
export default function Settings() {
  ...
  const { data: badgeData, isLoading: badgeLoading } = useQuery({
    ...
    enabled: !!user,
    staleTime: 5_000,          // low stale time so XP/level reflects quickly after socket invalidation
    refetchInterval: 30_000,   // poll every 30s so XP changes are reflected without a full refresh
  });
  ...
  // Callback ref so BadgeSection can advance its own carousel after a successful evolve.
  const badgeCarouselAdvanceRef = useRef<((newLevel: number) => void) | null>(null);
  ...
  const evolveBadgeMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["badge-me"] });
      if (user?.id) queryClient.invalidateQueries({ queryKey: ["badge-user", user.id] });
      // Auto-advance carousel to the newly reached level (0-indexed: level - 1)
      if (data?.level != null && data.level > 0) {
        badgeCarouselAdvanceRef.current?.(data.level - 1);
      }
      // Scroll the settings page back to the top so the badge carousel
      // (which is near the top of Settings) is immediately visible
      setTimeout(() => {
  ...
  const sectionScrollPos = useRef<Record<string, number>>({});

  // When entering a sub-section: save main scroll position, then scroll to top.
  // When going back to main: restore the saved position.
  const prevActiveSection = useRef(activeSection);
  useEffect(() => {
    const prev = prevActiveSection.current;
    prevActiveSection.current = activeSection;

    if (activeSection === "main") {
      // Save sub-section scroll before leaving
      if (scrollRef.current) {
        sectionScrollPos.current[prev] = scrollRef.current.scrollTop;
      }
      // Restore main scroll position
      const saved = sectionScrollPos.current["main"] ?? 0;
      requestAnimationFrame(() => {
        if (scrollRef.current) scrollRef.current.scrollTop = saved;
      });
    } else {
      // Entering a sub-section — save where we came from, scroll to top
      if (scrollRef.current) {
        sectionScrollPos.current[prev] = scrollRef.current.scrollTop;
        scrollRef.current.scrollTop = 0;
      }
      // Restore this sub-section's previously saved scroll position
      const savedSub = sectionScrollPos.current[activeSection] ?? 0;
      if (savedSub > 0) {
        requestAnimationFrame(() => {
          if (scrollRef.current) scrollRef.current.scrollTop = savedSub;
        });
      }
    }
  }, [activeSection]);
        ...
        <div className="px-4 pt-6 pb-2 space-y-4">
          ...
          {user && (
            <BadgeSection
              ...
              onRegisterAdvance={(fn) => { badgeCarouselAdvanceRef.current = fn; }}
            />
          )}
          ...
          <div className="bg-secondary rounded-2xl overflow-hidden border border-border divide-y divide-border">
            ...
            <div className="w-full flex items-center gap-3 px-4 py-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">{lang === "th" ? t.langEn : t.langTh}</p>
              </div>
              <button
                ...
                className={cn(
                  "w-11 h-6 rounded-full transition-colors flex items-center px-0.5 flex-shrink-0 focus:outline-none",
                  lang === "en" ? "bg-foreground" : "bg-border"
                )}
              >
                <div className={cn(
                  "w-5 h-5 rounded-full bg-white shadow transition-transform",
      ...
      {privacyDialogOpen && createPortal(
        <>
          ...
          <div
            ...
            style={{ left: "50%", transform: "translateX(-50%)", width: "min(100%, 430px)", paddingBottom: "max(var(--sai-bottom), 20px)" }}
                  ...
                  <div key={album.id} className="flex items-center gap-3 bg-secondary rounded-2xl p-3 border border-border">
                    <div className="flex -space-x-2 flex-shrink-0">
                      {album.posters.slice(0, 2).map((poster, i) => (
                        <div key={i} className="relative w-10 h-14 rounded-xl overflow-hidden bg-zinc-900 border border-border" style={{ zIndex: 2 - i }}>
                          <PosterImage src={poster} alt="" fallbackIcon={<Film className="w-4 h-4 text-muted-foreground" />} />
                        </div>
                      ))}
          ...
          {!activitiesLoading && (() => {
            ...
            const ActivityRow = ({ posterUrl, title, label, timeStr, comment, icon, onClick }: {
              ...
              <div className="flex items-start gap-3 bg-secondary rounded-2xl p-3 border border-border active:opacity-70 cursor-pointer" onClick={onClick}>
                <div className="relative w-10 h-14 rounded-xl overflow-hidden bg-zinc-900 flex-shrink-0 border border-border">
                  <PosterImage src={posterUrl} alt={title} fallbackIcon={<Film className="w-4 h-4 text-muted-foreground" />} />
                </div>
                <div className="flex-1 min-w-0">
