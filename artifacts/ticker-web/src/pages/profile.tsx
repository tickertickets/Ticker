...
function SortableCreatedChainCard({ chain, isOwn, statusBadge, onLongPress, onLongPressEnd }: {
  chain: ChainProfile; isOwn: boolean;
  statusBadge?: React.ReactNode;
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...(isOwn ? listeners : {})}>
      <ProfileChainCard
...
function ChainTabContent({
  ...
  useEffect(() => {
    ...
    if (chainManuallyReordered.current && sameSet) {
      // User has manually reordered and the set of chains is unchanged — preserve display order
      prevChainIdsRef.current = newIds;
      return;
    }
    // Either first load or chain set changed (added/removed) — sync from server
  ...
  const chainDragSensors = useSensors(
    ...
    useSensor(TouchSensor, { activationConstraint: { delay: 600, tolerance: 12 } }),
  );
      ...
      <div style={{ display: chainSubTab === "played" ? "block" : "none" }}>
        {(playedRunsData?.runs ?? []).length === 0 && !playedLoading ? (
          ...
          <div className="grid grid-cols-3 gap-2 px-3 pt-2 pb-2">
            {(playedRunsData?.runs ?? []).filter(run => isOwn || !(run.chain as any).isPrivate).map(run => {
...
export default function Profile() {
  ...
  }, [username, queryClient, scrollRef]);
