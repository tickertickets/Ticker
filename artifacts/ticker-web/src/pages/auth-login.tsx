...
export default function AuthLogin() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      // 2) Soft SPA navigate — safe because refreshUser() already:
      //    a) removed the stale 401 query-cache entry,
      //    b) fetched fresh user data via fetchQuery (writes to query cache),
      //    c) called setCachedUser(fresh) updating AuthContext state directly.
      //    By the time navigate() fires, AuthContext's `user` is already `fresh`,
      //    so there is no guest-flash and no React state race.
      //    Using wouter navigate() instead of window.location.replace() avoids
      //    a full page reload which would briefly show the address bar in PWA
      //    standalone mode.
      navigate(fresh.isOnboarded ? "/" : "/onboarding");
      return;
    } catch {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Content */}
        <div className="flex-1 flex flex-col px-5 overflow-y-auto" style={{ paddingBottom: "var(--sai-bottom)" }}>
