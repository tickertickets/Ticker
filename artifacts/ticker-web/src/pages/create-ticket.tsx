...
function TicketMovieRow({ movie, onSelect, ariaLabel }: { movie: TicketMovieItem; onSelect: (imdbId: string, movie: TicketMovieItem) => void; ariaLabel: (name: string) => string }) {
  ...
  return (
    <div className={`w-full flex items-center gap-3 p-3 rounded-2xl border border-border transition-colors ${isUnreleased ? "bg-secondary/50 opacity-60" : "bg-background active:bg-secondary"}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {isUnreleased && (
          <p className="text-[10px] font-semibold text-muted-foreground mt-0.5">{t.comingSoon}</p>
        )}
        {!isUnreleased && (
          <div className="mt-1">
      ...
      <button
        onClick={() => !isUnreleased && onSelect(movie.imdbId, movie)}
        disabled={isUnreleased}
        className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 transition-transform ${isUnreleased ? "bg-border cursor-not-allowed" : "bg-foreground active:scale-95"}`}
        aria-label={ariaLabel(movie.title)}
      >
        <span className={`text-sm font-bold ${isUnreleased ? "text-muted-foreground" : "text-background"}`}>+</span>
      </button>
    </div>
  );
}
...
export default function CreateTicket() {
  ...
  const [showTamperingWarning, setShowTamperingWarning] = useState(false);
  ...
  const { data: trendingPage2 } = useQuery({
    ...
    queryFn: () => fetch("/api/movies/trending?page=2", { credentials: "include" }).then(r => r.json()),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const { data: movieDetails } = useQuery<MovieDetails>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/movies/${encodeURIComponent(selectedMovieId!)}?lang=${selectedLang}`, { credentials: "include" });
  ...
  useEnsureMovieCores([
    ...trendingSuggestions.map((m: TicketMovieItem) => m.imdbId),
    ...movies.map((m: TicketMovieItem) => m.imdbId),
  ]);
  ...
  const { data: seasonsData } = useQuery<{
    ...
    queryFn: async () => {
      const res = await fetch(`/api/movies/${encodeURIComponent(selectedMovieId!)}/seasons`, { credentials: "include" });
  ...
  // Parse episode label like "S01E03 · Name" → { season:1, episode:3 }
  // or "S01 · Season Name" (season-only) → { season:1, episode:null }
  ...
  const { data: backdropsData, isLoading: backdropsLoading } = useQuery<{ images: CoverImage[] }>({
    queryKey: ["/api/movies/backdrops", selectedMovieId, parsedEp?.season ?? null, parsedEp?.episode ?? null],
    queryFn: async () => {
      const url = new URL(`/api/movies/${encodeURIComponent(selectedMovieId!)}/backdrops`, window.location.origin);
  ...
  useEffect(() => {
    ...
    coverImages.forEach(img => { const el = new Image(); el.src = img.url; });
  }, [coverImages]);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      cardTheme, selectedBackdropUrl, cardOffsetX, isPrivate, isPrivateMemory, hideRating, isSpoiler,
  ...
  const addInvitee = useCallback((u: UserSearchResult) => {
    ...
    // Guards MUST be re-evaluated inside the functional updater so they always
    // see the latest committed state — not a stale closure snapshot.  When the
    // user taps several friends in quick succession React batches the outer
    // renders, so `partyInvitees` / `partySize` captured in the closure can be
    // stale, causing the duplicate-check and cap-check to both pass for friends
    // that should have been blocked, or conversely to block friends that should
    // have been accepted.  Moving the checks inside the updater (which always
    // receives the latest prev value) fixes the stale-closure race.
    setPartyInvitees(prev => {
      if (prev.find(p => p.id === u.id)) return prev;        // already added
      if (prev.length >= partySize - 1) return prev;         // at cap
      return [...prev, u];
    });
    setUserSearchQuery("");
  }, [user, partySize]);
  ...
  const handleSubmit = async () => {
    ...
    // Strip invisible/zero-width unicode before validation
    const _stripInvisible = (s: string) => s.replace(/[\u00AD\u034F\u200B-\u200F\u2028-\u202F\u205F-\u206F\u3164\uFEFF]/g, "").trim();
    const strippedMemory = _stripInvisible(memoryNote);
    if (memoryNote.trim().length > 0 && strippedMemory.length === 0) {
      setShowTamperingWarning(true);
      return;
    }
    if (strippedMemory.length === 0) {
      setSubmitError(t.errNoMemory);
      return;
    }
    if (strippedMemory.length < 5) {
      setSubmitError(t.errMemoryTooShort);
      return;
    }
    ...
    // Track whether the API call succeeded so post-success cleanup errors
    // don't trigger a false error toast (issue 1.5.1 / 1.5.2).
    // The server may have already created the post even if the client-side
    // cleanup (cache invalidation, navigation) later throws — in that case
    // we navigate home and the post appears in the feed normally.
    ...
    try {
      ...
      mutationSucceeded = true; // server confirmed — cleanup errors below are non-fatal
      ...
      if (user?.username) {
        queryClient.setQueriesData<TicketListResponse>(
          ...
          (old: TicketListResponse | undefined) => old
      ...
      if (mutationSucceeded) {
        // Post was created on the server but a cleanup step (cache invalidation,
        // navigation, etc.) threw an error — navigate home so the user can see
        // their post rather than being stuck with a false error message.
        setLocation("/");
        return;
      }
      const errData = (err as any)?.data ?? {};
      const errCode: string = typeof errData?.error === "string" ? errData.error : "";
      ...
      } else if (errCode === "movie_not_released" || errMsg.includes("movie_not_released")) {
        setSubmitError(t.errUnreleasedMovie);
      } else if (errCode === "rate_limited" || errMsg.includes("rate_limited") || (err as any)?.status === 429) {
        // Show the server's human-readable rate-limit message when available,
        // otherwise fall back to a generic "too fast" message.
        const serverMsg: string = typeof errData?.message === "string" ? errData.message : "";
        setSubmitError(serverMsg || (t.errRateLimit ?? t.errGeneric));
      } else {
    ...
    <div ref={scrollRef} className="h-full overflow-y-auto overscroll-y-none">
      ...
      {step === 1 && (
        <div className="px-4 pt-4 pb-2">
          ...
          <div className="flex flex-col gap-2">
            {(movies as TicketMovieItem[]).map((movie) => (
              <TicketMovieRow key={movie.imdbId} movie={movie} onSelect={handleSelectMovie} ariaLabel={t.selectMovieAria} />
            ))}
          </div>
        </div>
      ...
      {step === 2 && movieDetails && (
        <div className="pb-4">
          ...
          <div className="px-4 space-y-5">
            ...
            <div>
              ...
              {cardTheme === "poster" && (
                <div className="mt-3 space-y-3">
                  ...
                  {selectedBackdropUrl && (
                    <p className="text-[10px] text-muted-foreground text-center">{t.dragToAdjust}</p>
                  )}
                </div>
              )}
            </div>
            ...
            {/* ── ความทรงจำ (required) ── */}
            <div>
              <p className="text-xs font-black tracking-widest text-foreground mb-2">
                {t.memoryLabel}
              </p>
              <textarea
            ...
            <div className="flex items-center justify-between py-3 px-4 bg-secondary rounded-2xl">
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">{t.privateCardDesc}</p>
              </div>
              <button onClick={() => setIsPrivate(v => !v)}
                className={cn("shrink-0 w-11 h-6 rounded-full transition-colors relative", isPrivate ? "bg-foreground" : "bg-border")}>
            ...
            {submitError && (
              <div className="flex flex-col items-center gap-1">
                <p className="text-sm text-red-500 text-center font-semibold">{submitError}</p>
                {(submitError === t.errNoRating || submitError === t.errNoMemory || submitError === t.errMemoryTooShort) && (
                  <p className="text-xs text-muted-foreground text-center">{t.errNotWatchedYet}</p>
                )}
              </div>
            )}
        ...
        return createPortal(
          ...
          >
            ...
            <div
              ...
              style={{ boxShadow: "0 -4px 32px rgba(0,0,0,0.22)", paddingBottom: "var(--sai-bottom)" }}
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-2">
                ...
                <span className="font-bold text-base text-foreground">{thMonths[pickerMonth]}</span>
                <button
                  onClick={nextMonth}
      ...
      {/* ── Tampering Warning ── */}
      {showTamperingWarning && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-end" onClick={() => setShowTamperingWarning(false)}>
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" />
          <div
            className="relative w-full bg-background rounded-t-3xl border-t border-border px-5 pt-5"
            style={{ boxShadow: "0 -4px 32px rgba(0,0,0,0.18)", paddingBottom: "max(1.5rem, var(--sai-bottom) + 1rem)" }}
          ...
          >
            ...
            <p className="text-base font-bold text-foreground mb-2 text-center">{t.warnTamperingTitle}</p>
            <p className="text-sm text-muted-foreground mb-5 text-center leading-relaxed">{t.warnTamperingBody}</p>
            <button
              onClick={() => setShowTamperingWarning(false)}
              className="w-full h-12 rounded-2xl bg-foreground text-background font-bold text-sm"
            >
              OK
            </button>
