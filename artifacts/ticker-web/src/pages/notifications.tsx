...
import { Loader2, Heart, MessageCircle, MessagesSquare, UserPlus, Users, Sparkles, X, Lock, Unlock, ArrowLeft, Search, Bell, Trash2, Link2 } from "lucide-react";
...
function PartyAcceptModal({
  ...
  inviteId: string;
  onClose: () => void;
  onSuccess: (result: { accepted: boolean }) => void;
}) {
  ...
  const accept = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/party/invite/${inviteId}/accept`, {
        ...
        body: JSON.stringify({ seatNumber, rating, ratingType: isDyingStar ? "blackhole" : "star", hideRating }),
      });
      const body = await res.json();
      if (!res.ok) throw body;
      return body;
    },
    onSuccess: () => { onSuccess({ accepted: true }); onClose(); },
  ...
  const decline = useMutation({
    ...
    onSuccess: () => { onSuccess({ accepted: false }); onClose(); },
  });
        ...
        {!isLoading && !isExpired && (
          <div className="px-5 space-y-5" style={{ paddingBottom: "3rem" }}>
            ...
            <div>
              ...
              {/* Hide rating toggle — ผู้รับเชิญเลือกเองว่าจะซ่อนดาวหรือไม่ */}
              <button
                type="button"
                role="switch"
                aria-checked={hideRating}
                onClick={() => setHideRating(v => !v)}
                className="flex items-center gap-2 select-none active:opacity-70 mt-2"
                style={{ background: "none", border: "none", padding: 0, cursor: "pointer" }}
              >
                <div className={cn(
                  "w-11 h-6 rounded-full transition-colors flex items-center px-0.5 flex-shrink-0",
                ...
                <span className="text-sm font-bold text-foreground">{t.hideRatingLabel}</span>
              </button>
            </div>
...
function getNotifIcon(type: string) {
  switch (type) {
    case 'like': return <Heart className="w-3.5 h-3.5 fill-current text-foreground" />;
    case 'comment': return <MessagesSquare className="w-3.5 h-3.5 text-foreground" />;
    case 'comment_reply': return <MessagesSquare className="w-3.5 h-3.5 text-foreground" />;
    case 'chain_comment': return <MessagesSquare className="w-3.5 h-3.5 text-foreground" />;
    case 'chain_comment_reply': return <MessagesSquare className="w-3.5 h-3.5 text-foreground" />;
    case 'wiki_comment_reply': return <MessagesSquare className="w-3.5 h-3.5 text-foreground" />;
    case 'follow': return <UserPlus className="w-3.5 h-3.5 text-foreground" />;
    case 'follow_request': return <UserPlus className="w-3.5 h-3.5 text-foreground" />;
    case 'party_invite': return <Users className="w-3.5 h-3.5 text-foreground" />;
    case 'party_color_unlock': return <Sparkles className="w-3.5 h-3.5 text-foreground" />;
    case 'memory_request': return <Lock className="w-3.5 h-3.5 text-foreground" />;
    case 'memory_approved': return <Unlock className="w-3.5 h-3.5 text-foreground" />;
    case 'chain_continued': return <Link2 className="w-3.5 h-3.5 text-foreground" />;
    case 'chain_run_started': return <Link2 className="w-3.5 h-3.5 text-foreground" />;
    default: return null;
  }
}
...
export default function Notifications() {
  ...
  const handleInviteSuccess = ({ accepted }: { accepted: boolean }) => {
    queryClient.invalidateQueries({ queryKey: ['/api/notifications'] });
    queryClient.invalidateQueries({ queryKey: ['notifications-unread-count'] });

    if (accepted) {
      // Accepting a Tickets Party invite creates a new ticket in the feed —
      // take the user there (instead of leaving them stranded on the
      // notifications page) and make sure the feed re-fetches so the new
      // ticket shows up immediately.
  ...
  const notifications = searchQuery
    ? allNotifications.filter((n: NotifItem) =>
          ...
          {notifications.map((notif: NotifItem) => {
            ...
            return (
              <div
                key={notif.id}
                className={cn(
                  "flex items-start gap-3 px-4 py-3.5 border-b border-border/40 transition-colors cursor-pointer",
                  !notif.isRead && "bg-secondary/40"
                )}
                onClick={() => {
                  if (!notif.isRead) markOneRead(notif.id);
                  const type = notif.type as string;
                  const commentId = notifRec["commentId"] as string | undefined;
                  if (notif.ticketId && type !== "party_invite" && type !== "memory_request" && type !== "party_color_unlock" && type !== "follow" && type !== "follow_request") {
                    navigate(`/post/ticket/${notif.ticketId}${commentId ? `?commentId=${commentId}` : ""}`);
                  } else if (notifRec["chainId"] && (type === "chain_continued" || type === "chain_run_started" || type === "chain_comment" || type === "chain_comment_reply" || type === "chain_like")) {
                    navigate(`/post/chain/${notifRec["chainId"]}${commentId ? `?commentId=${commentId}` : ""}`);
              ...
              >
                ...
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  {notif.ticketId && !isPartyInvite && (notif.type as string) !== "memory_request" && (notifRec["ticketPosterUrl"] || isColorNotif) && (
                    <Link href={`/post/ticket/${notif.ticketId}`} onClick={e => e.stopPropagation()}>
                      <div className="relative w-11 h-14 rounded-xl overflow-hidden border border-border flex items-center justify-center bg-secondary">
                        {isColorNotif
                          ? <Sparkles className="w-4 h-4 text-muted-foreground" />
                          : <PosterImage src={notifRec["ticketPosterUrl"] as string | null | undefined} alt="" />}
                      </div>
                    </Link>
                  )}
                  {!!notifRec["chainId"] && ((notif.type as string) === "chain_continued" || (notif.type as string) === "chain_run_started") && (
                    <Link href={`/post/chain/${notifRec["chainId"]}`} onClick={e => e.stopPropagation()}>
