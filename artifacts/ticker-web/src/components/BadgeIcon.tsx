...
const BADGE_META: Record<number, BadgeMeta> = {
  ...
  5: {
    ...
    level: 5, name: "Connoisseur", nameTH: "เซียนหนัง",
    mainColor: "#c0b0e8", lightColor: "#eee8ff", darkColor: "#483070",
    outlineColor: "#1c0840", glowColor: "rgba(180,160,230,0.80)",
    description: "Your taste in film is unmatched.", descriptionTH: "เซียนหนังตัวจริง ไม่มีใครเทียบรสนิยม",
  },
};
...
function TicketShape({ level, size = 16, flat = false }: { level: number; size?: number; flat?: boolean }) {
  // useId ensures each SVG instance gets unique gradient/filter IDs,
  // preventing browser cross-SVG gradient ID collision on pages with many badges.
  ...
  return (
    ...
    >
      <DrawTicket
        ...
        VW={VW} VH={VH} rot={flat ? 0 : -16} uid={reactId}
      />
    </svg>
  );
}
...
function TicketLarge({ level }: { level: number }) {
  ...
  return (
    ...
    >
      <DrawTicket
        level={level} W={W} H={H} cR={cR} nBumps={nBumps}
        VW={VW} VH={VH} rot={-14} uid={reactId}
      />
    </svg>
  );
}
