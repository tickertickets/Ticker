...
const BADGE_SHOWN_KEY = "push_badge_shown_v1";
...
function getBadgeShownSet(): Set<string> {
  try {
    const raw = localStorage.getItem(BADGE_SHOWN_KEY);
    if (!raw) return new Set();
    const arr = JSON.parse(raw);
    return new Set(Array.isArray(arr) ? arr : []);
  } catch {
...
function markBadgeShown(userId: string) {
  const s = getBadgeShownSet();
  s.add(userId);
  try {
    localStorage.setItem(BADGE_SHOWN_KEY, JSON.stringify(Array.from(s)));
  } catch { /* ignore */ }
}
...
export function PushPermissionPrompt() {
  ...
  const maybeShowBadge = () => {
    if (!user) return;
    if (!getBadgeShownSet().has(user.id)) {
      markBadgeShown(user.id);
      setBadgeOpen(true);
    }
  };
  ...
  return (
    <>
      ...
      {/* Badge Lv1 follow-up modal — shown once after first push decision */}
      {badgeOpen && (
        <div
          className="fixed inset-0 z-[201] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200"
        ...
        >
          ...
          >
            <div className="flex flex-col items-center text-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-foreground flex items-center justify-center">
                <Ticket className="w-7 h-7 text-background" strokeWidth={2.2} />
              </div>
              <h2 className="font-display font-bold text-xl text-foreground">
                {t.pushBadgePromptTitle}
              </h2>
              <p className="text-sm text-muted-foreground leading-relaxed px-2">
                {t.pushBadgePromptBody}
              </p>
            </div>
            <div className="mt-6 flex flex-col gap-2">
              <button
                type="button"
                onClick={() => { setBadgeOpen(false); navigate("/settings"); }}
                className="w-full h-12 rounded-full bg-foreground text-background font-semibold text-sm active:scale-[0.98] transition-transform"
              >
                {t.pushBadgePromptBtn}
              </button>
              <button
                type="button"
                onClick={() => setBadgeOpen(false)}
