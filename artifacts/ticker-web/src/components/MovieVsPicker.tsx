...
function MovieRow({
  ...
  return (
    <div className="flex items-center gap-3 bg-secondary rounded-2xl px-3 py-2.5 border border-border">
      <div className="relative w-12 flex-shrink-0 rounded-xl overflow-hidden bg-zinc-900 border border-border/50" style={{ height: 68 }}>
        <PosterImage src={m.posterUrl} alt={m.title} fallbackIcon={<Film className="w-3 h-3 text-muted-foreground" />} />
      </div>
...
export function MovieVsPicker({ onClose }: { onClose: () => void }) {
  ...
  const s = {
    title:        lang === "th" ? "ให้ระบบเลือก"                : "Let the System Pick",
  ...
  const { data: trendingData } = useQuery<{ movies: VsMovie[] }>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/movies/trending?page=1&lang=${apiLang}`, { credentials: "include" });
  ...
  useEffect(() => {
    ...
    setSearching(true);
    fetch(`/api/movies/search?query=${encodeURIComponent(debouncedQuery)}&lang=${apiLang}`, { credentials: "include", signal: ctrl.signal })
      .then(r => r.json())
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ maxHeight: maxH, paddingBottom: "var(--sai-bottom)" }}
      >
        ...
        {step === "loading" && (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            <div className="w-20 h-20 rounded-2xl bg-secondary flex items-center justify-center">
              <Swords className="w-10 h-10 text-foreground animate-pulse" />
            </div>
        ...
        {step === "result" && winner && (() => {
          ...
          return (
            <div className="flex flex-col" style={{ paddingBottom: "calc(var(--sai-bottom) + 1.5rem)" }}>
              ...
              <div className="px-4">
                <div className="relative rounded-2xl overflow-hidden bg-zinc-900 border border-border" style={{ aspectRatio: "16/9" }}>
                  <PosterImage src={winner.posterUrl} alt={winner.title} objectPosition="center 20%" eager fallbackIcon={<Film className="w-10 h-10 text-zinc-600" />} />
        ...
        {/* ── Pick step ── */}
        {step === "pick" && (
          <div className="flex flex-col overflow-hidden" style={{ maxHeight: `calc(${maxH} - 40px)` }}>
            {/* Header */}
            ...
            {picked.length > 0 && (
              <div className="px-4 mb-3">
                ...
                <div className="flex gap-2 overflow-x-auto scrollbar-hide pt-4 pb-2">
                  {picked.map(m => (
