...
import { Loader2, Link2, Heart, Share2, X, Trash2, Users, Search, Bookmark, Flag, Send, MessageCircle, MessagesSquare, Check, CornerDownRight, EyeOff, Eye, Pencil } from "lucide-react";
...
import { PosterImage, isPosterLoaded, markPosterLoaded } from "@/components/PosterImage";

// ── Track whether ALL poster URLs in a collage have finished loading ─────────
// Returns true immediately if all are already cached; otherwise preloads each
// missing URL and flips to true once the last one completes/errors.
function useAllPostersLoaded(posters: string[]): boolean {
  const [ready, setReady] = useState(() => posters.length > 0 && posters.every(isPosterLoaded));

  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    if (posters.length === 0) { setReady(true); return; }
    if (posters.every(isPosterLoaded)) { setReady(true); return; }
    setReady(false);
    let cancelled = false;
    const pending = posters.filter(p => !isPosterLoaded(p));
  ...
  }, [posters.join(",")]); // eslint-disable-line react-hooks/exhaustive-deps
...
export function ChainCommentSheet({ chainId, onClose, commentCount: initialCommentCount = 1, onCommentAdded, onCommentDeleted }: { chainId: string; onClose: () => void; commentCount?: number; onCommentAdded?: () => void; onCommentDeleted?: () => void }) {
  ...
  const handleEditChainComment = async (commentId: string) => {
    if (!editContent.trim()) return;
    try {
      const res = await fetch(`/api/chains/${chainId}/comments/${commentId}`, {
        method: "PATCH",
        credentials: "include",
      ...
      if (res.ok) {
        qc.setQueryData<{ comments: ChainComment[] }>([`chain-comments-${chainId}`], (old) =>
          old ? { comments: old.comments.map(c => c.id === commentId ? { ...c, content: editContent.trim() } : c) } : old
        );
        // อัพเดท preview bubble บนการ์ดด้วย
        qc.invalidateQueries({ queryKey: [`chain-comments-preview-${chainId}`] });
  ...
  const renderComment = (c: ChainComment, isReply = false) => {
    ...
    return (
      <div key={c.id} className={cn("flex gap-3 px-4 py-2.5", isReply && "pl-10")}>
        {isReply && <div className="w-px self-stretch bg-border mr-0 -ml-5 flex-shrink-0" />}
        <div
          className="cursor-pointer flex-shrink-0"
          onClick={() => { onClose(); navigate(`/profile/${c.username}`); }}
        >
          <div className={cn("rounded-lg bg-black overflow-hidden flex items-center justify-center border border-white/10", isReply ? "w-7 h-7" : "w-8 h-8")}>
            {c.avatarUrl
        ...
        <div className="flex-1 min-w-0">
              ...
              confirmDeleteCommentId === c.id ? (
                <div className="ml-auto flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="text-[10px] font-bold text-destructive px-1.5 py-0.5 rounded-lg bg-destructive/10"
                  >ลบ</button>
                </div>
              ) : (
                <div className="ml-auto flex items-center gap-0.5">
                  {editingCommentId !== c.id && (
                    <button onClick={() => { setEditContent(c.content); setEditingCommentId(c.id); }} className="p-1 text-muted-foreground/40 hover:text-foreground transition-colors">
          ...
          <div className="bg-secondary rounded-2xl rounded-tl-sm px-3.5 py-2.5">
            <p className="text-sm text-foreground/90 leading-relaxed break-words" style={{ overflowWrap: "break-word", wordBreak: "break-word" }}>{renderCommentContent(c.content)}</p>
          </div>
          {editingCommentId === c.id && (
            <div className="mt-2 flex gap-2">
              <textarea
                className="flex-1 bg-secondary rounded-xl px-3 py-2 text-sm text-foreground resize-none outline-none border border-border/50 focus:border-border"
                value={editContent}
              ...
              <div className="flex flex-col gap-1 shrink-0">
                <button onClick={() => handleEditChainComment(c.id)} className="px-3 py-1.5 bg-foreground text-background text-[10px] font-bold rounded-xl">บันทึก</button>
                <button onClick={() => { setEditingCommentId(null); setEditContent(""); }} className="px-3 py-1.5 bg-secondary text-muted-foreground text-[10px] rounded-xl">ยกเลิก</button>
              </div>
            </div>
          )}
          {!isReply && user && (
            <button
              onClick={() => {
                setReplyingTo({ id: c.id, username: c.username ?? c.displayName ?? "?" });
          ...
          {commentsLoading ? (
            ...
            rootComments.map(c => (
              <div key={c.id}>
                ...
                {(repliesMap.get(c.id) ?? []).map(reply => renderComment(reply, true))}
              </div>
            ))
          )}
        </div>
        {user && (
          <div className="flex flex-col px-4 pt-2 border-t border-border flex-shrink-0" style={{ paddingBottom: "max(12px, var(--sai-bottom) + 8px)" }}>
            {replyingTo && (
              <div className="flex items-center gap-1.5 mb-1.5 px-1">
                <CornerDownRight className="w-3 h-3 text-muted-foreground" />
                <span className="text-xs text-muted-foreground">{lang === "th" ? "ตอบ" : "Reply to"} @{replyingTo.username}</span>
                <button type="button" onClick={() => setReplyingTo(null)} className="ml-auto text-muted-foreground hover:text-foreground text-xs">✕</button>
              </div>
            )}
...
function ChainCommentBubble({ chainId, commentCount }: { chainId: string; commentCount: number }) {
  const { data } = useQuery<{ comments: ChainComment[] }>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/chains/${chainId}/comments?limit=6`, { credentials: "include" });
      if (!res.ok) return { comments: [] };
      return res.json();
    },
    staleTime: 0,
  ...
  return (
    <div className="mx-4 mb-3 px-3 py-2 bg-secondary/60 rounded-2xl flex items-start gap-2">
      ...
      <div
        className="flex items-center gap-1 flex-1 overflow-hidden transition-opacity duration-300"
        style={{ opacity: show ? 1 : 0 }}
      >
        <span className="text-xs font-semibold text-foreground/85 shrink-0 max-w-[40%] truncate">{c.displayName || c.username || ""}</span>
        {isVerified(c.username) && <VerifiedBadge className="w-2.5 h-2.5 flex-shrink-0" />}
        {c.userId && <BadgeIcon userId={c.userId} size={10} />}
        <span className="text-xs text-foreground/70 truncate">{c.content}</span>
      </div>
    </div>
  );
}
...
export function PosterCollage({ posters, emptyIcon, eager }: { posters: string[]; emptyIcon?: React.ReactNode; eager?: boolean }) {
  const count = posters.length;
  // Wait until every poster image has fully loaded before revealing the
  // collage — prevents the "checkerboard" effect where cells pop in one by one.
  const allLoaded = useAllPostersLoaded(posters);
  ...
  const content = (() => {
    ...
    if (count === 1) {
      return <PosterImage src={posters[0]} eager={eager} />;
    }
    if (count <= 3) {
      return (
        <div className="absolute inset-0 flex gap-px bg-black">
          {posters.map((url, i) => (
            <div key={i} className="flex-1 relative overflow-hidden bg-secondary">
              <PosterImage src={url} eager={eager} />
            </div>
          ))}
        </div>
      );
  ...
  return (
    <>
      {content}
      {/* Single-block skeleton overlaid until ALL poster images finish loading.
          Fades away as one unit instead of cells popping in one by one. */}
      {count > 0 && !allLoaded && (
        <div
          className="absolute inset-0 z-10 bg-secondary skeleton-shimmer"
      ...
      >
        ...
        <div className="px-5 pt-5">
          ...
          <p className="text-center text-[10px] text-muted-foreground/50 pt-4">
            © Ticker · ต้องการแจ้ง DMCA? กดปุ่ม <Flag className="w-2.5 h-2.5 inline-block" /> ที่โพสต์นั้น
          </p>
        </div>
      </div>
    </>,
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          maxHeight: "80vh",
          overflow: "hidden",
          paddingBottom: "max(var(--sai-bottom), 8px)",
        }}
        onClick={e => e.stopPropagation()}
      >
...
// Memoized for the same reason as TicketCard — avoids re-rendering every
// visible card on unrelated feed state changes (scroll header, PTR, etc.).
export const ChainCard = memo(function ChainCard({ chain, onNotInterested }: { chain: ChainItem; onNotInterested?: () => void }) {
  const { t, lang } = useLang();
  const { user } = useAuth();
  const qc = useQueryClient();
  const { toast } = useToast();
    ...
    <>
      <div className="bg-background pb-4 border-b border-border/50">
        ...
        <div className="flex items-center gap-2.5 pl-0 pr-4 pt-3 pb-3">
          ...
          {!isOwner && !isVerified(chain.user?.username) ? (
          ...
          {isOwner && linkSheetOpen && (
            <AddLinkSheet
              ...
              onSaved={(newLinks) => {
                ...
                syncChainLinks(chain.id, newLinks);
              }}
            />
          )}
          {tags.length > 0 && (
        ...
        {/* Post timestamp */}
        {chain.createdAt && (
          <p className="mt-2 text-[11px] text-muted-foreground/60 leading-none text-center">
            {timeAgo(chain.createdAt, lang)}
          </p>
        )}
...
// ── ChainsSection — infinite scroll (same pattern as TicketsFeed) ─────────────
//
// Page 1 fetched via useQuery (preserves "chains-feed" cache key).
// Extra pages fetched manually with an offset cursor.
...
export function ChainsSection() {
  ...
  const { data, isLoading } = useQuery<{ chains: ChainItem[]; hasMore?: boolean; nextCursor?: string | null }>({
    queryKey: ["chains-feed"],
    queryFn: async () => {
      const res = await fetch("/api/chains?limit=20", { credentials: "include" });
      if (!res.ok) throw new Error("failed");
      return res.json();
    },
    staleTime: 1000 * 60 * 2,
  ...
  const [extraPages, setExtraPages] = useState<ChainItem[][]>([]);
  const [cursor, setCursor] = useState<string | null>(null);
  const [hasMore, setHasMore] = useState(false);
  const [isFetchingMore, setIsFetchingMore] = useState(false);
  const sentinelRef = useRef<HTMLDivElement>(null);
  // Dedup guard: track all shown chain IDs so page-boundary overlap never
  // produces duplicate cards (can happen if score drift between two requests
  // re-surfaces an item at a different offset). Same pattern as TicketsFeed.
  ...
  useEffect(() => {
    if (data) {
      shownIdsRef.current = new Set((data.chains ?? []).map((c) => String(c.id)));
      setExtraPages([]);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/chains?limit=20&cursor=${encodeURIComponent(cursor)}`, { credentials: "include" });
      if (!res.ok) return;
      const json = await res.json();
      if (json.recycled) {
        setExtraPages(prev => {
          setRecycleStartIndex((current) => current ?? prev.flat().length);
          return [...prev, json.chains ?? []];
        });
      } else {
        const newChains: ChainItem[] = (json.chains ?? []).filter(
          (c: ChainItem) => !shownIdsRef.current.has(String(c.id)),
        );
        newChains.forEach((c) => shownIdsRef.current.add(String(c.id)));
        if (newChains.length > 0) setExtraPages(prev => [...prev, newChains]);
      }
  ...
  const firstPage: ChainItem[] = data?.chains ?? [];
  const chains: ChainItem[] = [firstPage, ...extraPages].flat();
      ...
      {isLoading ? (
        <div className="flex flex-col">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-background pb-4 border-b border-border/50 animate-pulse">
              {/* Creator row stub */}
              <div className="flex items-center gap-2.5 pl-0 pr-4 pt-3 pb-3">
                <div className="flex items-center gap-2 bg-secondary rounded-r-2xl pl-3 pr-3 py-1.5">
                  <div className="w-7 h-7 rounded-lg bg-secondary-foreground/10" />
                  <div className="space-y-1.5">
                    <div className="h-2.5 w-20 rounded-full bg-secondary-foreground/10" />
                    <div className="h-2 w-10 rounded-full bg-secondary-foreground/10" />
                  </div>
                </div>
              </div>
              {/* Poster collage stub */}
              <div className="flex justify-center px-4">
                <div style={{ width: 160, aspectRatio: "2/3", borderRadius: "0.75rem", overflow: "hidden" }}
                  className="bg-secondary" />
              </div>
              {/* Title + chain-count stubs */}
              <div className="px-4 mt-5 flex flex-col items-center gap-1.5">
                <div className="h-3 rounded-full bg-secondary" style={{ width: "55%" }} />
                <div className="h-2.5 rounded-full bg-secondary/60" style={{ width: "25%" }} />
              </div>
              {/* Timestamp stub */}
              <div className="flex justify-center mt-2">
                <div className="h-2 w-16 rounded-full bg-secondary/40" />
              </div>
              {/* Action bar stubs */}
              <div className="flex items-center justify-center gap-8 mt-4">
                {[0,1,2,3].map(j => <div key={j} className="w-5 h-5 rounded-full bg-secondary" />)}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col">
          {chains.map((c, i) => {
            // Recycled laps intentionally repeat the same underlying chain id,
