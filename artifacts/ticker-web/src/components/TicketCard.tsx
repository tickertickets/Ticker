...
import { useLang, displayYear, displayDate, timeAgo } from "@/lib/i18n";
...
function HeartBurst({ visible, onDone }: { visible: boolean; onDone: () => void }) {
  useEffect(() => {
    if (!visible) return;
...
function CompactCard({ ticket, onLongPress, viewHref, onDoubleTap }: { ticket: Ticket; onLongPress?: (t: Ticket) => void; viewHref?: string; onDoubleTap?: () => void }) {
  ...
  const { user } = useAuth();
  useEffect(() => { if (!flipped) setMemoryRevealed(false); }, [flipped]);
  // Reset flip when the feed refreshes
  useEffect(() => {
    const handler = () => setFlipped(false);
    window.addEventListener("feed-cards-reset", handler);
    return () => window.removeEventListener("feed-cards-reset", handler);
  }, []);
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    setFlipped(f => !f);
    if (diff > 0 && diff < 300 && onDoubleTap) {
      onDoubleTap();
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            background: isCompactPoster ? "#18181b" : "#111",
        ...
        >
          <div style={{ position: "absolute", top: 0, left: 0, width: CARD_SEED_W, height: CARD_SEED_H, transformOrigin: "top left", transform: `scale(${compactScale})` }}>
            <div className="absolute inset-0 p-3 flex flex-col">
              ...
              {(((ticket as unknown) as Record<string, unknown>)["isSpoiler"] === true) && !memoryRevealed ? (
                <div
                  className="flex-1 flex flex-col items-center justify-center gap-2 cursor-pointer"
                  onClick={(e) => { e.stopPropagation(); setMemoryRevealed(true); }}
                >
                ...
                {ticket.watchedAt && (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-[10px]" style={{ color: isCompactPoster ? "rgba(28,28,28,0.55)" : "var(--card-back-text-muted)" }}>
                      {displayDate(ticket.watchedAt, lang, { day: "numeric", month: "short", year: "numeric" })}
                    </span>
                  </div>
                )}
                {ticket.location && (
...
function ActionBtn({ onClick, children }: { onClick: (e: React.MouseEvent) => void; children: React.ReactNode }) {
  return (
    <button
      onClick={e => { onClick(e); (e.currentTarget as HTMLButtonElement).blur(); }}
...
function CommentBubble({ ticketId, commentCount }: { ticketId: string; commentCount: number }) {
  const { t } = useLang();
  const { data } = useQuery<{ comments: Array<{ id: string; userId: string | null; user: { displayName: string | null; username: string }; content: string }> }>({
    // Use the same queryKey prefix that useSocketTicketUpdates invalidates, so
    // deleting/editing a comment inside the detail page also refreshes this bubble.
  ...
  return (
    <div className="mx-4 mb-3 px-3 py-2 bg-secondary/60 rounded-2xl flex items-start gap-2">
      ...
      >
        <span className="text-xs font-semibold text-foreground/85 shrink-0 max-w-[40%] truncate">{c.user?.displayName ?? t.user}</span>
        {isVerified(c.user?.username) && <VerifiedBadge className="w-2.5 h-2.5 flex-shrink-0" />}
...
function CommentModal({ ticket, onClose }: { ticket: Ticket; onClose: () => void }) {
  ...
  const { data, refetch, isLoading: commentsLoading } = useQuery<{ comments: FeedComment[] }>({
  ...
  const rootComments = commentList.filter(c => !c.replyToId);
  ...
  const handleReply = (c: FeedComment) => {
    setReplyingTo({ id: c.id, username: c.user?.username ?? c.user?.displayName ?? "?" });
  ...
  const handleEditComment = async (commentId: string) => {
    ...
    if (!trimmed) return;
    try {
      await fetch(`/api/tickets/${ticket.id}/comments/${commentId}`, {
        method: "PATCH", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ content: trimmed }),
      });
      qc.setQueryData<{ comments: FeedComment[] }>([`/api/tickets/${ticket.id}/comments`], old =>
        old ? { comments: old.comments.map(x => x.id === commentId ? { ...x, content: trimmed } : x) } : old
      );
      qc.invalidateQueries({ queryKey: [`/api/tickets/${ticket.id}/comments`, "preview"] });
    ...
    return (
      <div key={c.id} className={cn("flex gap-3 px-4 py-2.5", isReply && "pl-10")}>
        ...
        <div
          ...
          onClick={() => { onClose(); navigate(`/profile/${c.user?.username}`); }}
        >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1 mb-1 flex-wrap">
            <span
              ...
              onClick={() => { onClose(); navigate(`/profile/${c.user?.username}`); }}
            >{c.user?.displayName || c.user?.username}</span>
            {isVerified(c.user?.username) && <VerifiedBadge className="w-3 h-3 flex-shrink-0" />}
            ...
            {user && user.id === c.userId && editingCommentId !== c.id && (
              confirmDeleteCommentId === c.id ? (
                ...
                <div className="ml-auto flex items-center gap-0.5">
                  <button
                    onClick={() => { setEditContent(c.content); setEditingCommentId(c.id); }}
          ...
          {editingCommentId === c.id && (
            <div className="mt-2 flex flex-col gap-1.5">
              <textarea
                ...
                className="w-full bg-secondary rounded-xl px-3 py-2 text-sm text-foreground outline-none resize-none border border-border focus:border-foreground/30 transition-colors"
                rows={2}
                autoFocus
              />
              <div className="flex gap-2 justify-end">
                <button onClick={() => handleEditComment(c.id)} className="px-3 py-1.5 bg-foreground text-background text-[10px] font-bold rounded-xl">บันทึก</button>
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-end justify-center" style={{ paddingBottom: keyboardHeight }} onClick={onClose}>
      ...
      >
        ...
        <div ref={listRef} className="flex-1 overflow-y-auto py-3 min-h-0">
          {commentsLoading ? (
...
function FeedCard({ ticket, onLongPress, onNotInterested }: { ticket: Ticket; onLongPress?: (t: Ticket) => void; onNotInterested?: () => void }) {
  ...
  const isOwner   = user?.id === ticket.userId;
  const { syncTicketLinks } = useLinkSync();
  ...
  }, [JSON.stringify((ticket as any).captionLinks ?? null)]);
  ...
  return (
    <div className="bg-background pb-4 border-b border-border/50" ref={cardExportRef}>
      ...
      <div className="flex items-center gap-2.5 pl-0 pr-4 pt-3 pb-3">
        ...
        {isOwner ? null : isVerified(ticket.user?.username) ? null : (
      ...
      {(() => {
        ...
        const isSpoilerCard = (((ticket as unknown) as Record<string, unknown>)["isSpoiler"] === true);
        ...
        return (
          <>
            ...
            {displayText && (
              isSpoilerCard && !captionRevealed ? (
                <button
                  className="px-4 mt-3 flex items-start gap-2 text-left w-full"
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); setCaptionRevealed(true); }}
                >
                  <EyeOff className="w-3.5 h-3.5 shrink-0 mt-0.5 text-red-400" />
                  <span className="text-xs font-semibold text-red-400">{t.spoilerHiddenMsg}</span>
                </button>
              ) : (
            ...
            {isOwner && linkSheetOpen && (
              <AddLinkSheet
                ...
                onSaved={(newLinks) => {
                  ...
                  syncTicketLinks(ticket.id, newLinks);
                }}
              />
            )}
          </>
      ...
      {/* Post timestamp — Instagram style: relative (< 24 h) then absolute date */}
      {ticket.createdAt && (
        <p className="px-4 mt-2 text-[11px] text-muted-foreground/60 leading-none">
          {timeAgo(ticket.createdAt, lang)}
        </p>
      )}

      {/* Comment cycling bubble — hidden for spoiler posts to avoid revealing spoiler content */}
      {(ticket.commentCount ?? 0) > 0 && !((ticket as unknown) as Record<string, unknown>)["isSpoiler"] && (
        <div className="mt-4">
...
export function CardContextMenu({ ticket, onClose }: CardMenuProps) {
  ...
  // Start with [] so the Pin button renders immediately (default state = "Pin").
  // The real pinned list loads async and updates the button icon/label.
...
// ── Main Export ───────────────────────────────────────────────────
// Memoized — feeds re-render this list on every scroll/state tick (header
// hide, pull-to-refresh, etc.); without memo every card re-renders too even
// though its own ticket data hasn't changed, which is a big share of the
// scroll jank on the feed and on profile collections.
// ─────────────────────────────────────────────────────────────────────────────
// FeedPostSkeleton — mirrors the real post layout (header + card + action bar)
// so the loading state looks like an actual post, just without real content.
// ─────────────────────────────────────────────────────────────────────────────
export function FeedPostSkeleton() {
  return (
    <div className="bg-background border-b border-border/50">
      {/* Creator row — matches pl-0 pr-4 pt-3 pb-3 of real post */}
      <div className="flex items-center gap-2.5 pl-0 pr-4 pt-3 pb-3">
        <div className="flex items-end gap-2 bg-secondary rounded-r-2xl pl-3 pr-3 py-1.5 flex-shrink-0">
          {/* Avatar square */}
          <div className="w-7 h-7 rounded-lg bg-muted-foreground/15 animate-pulse flex-shrink-0" />
          {/* Name + handle */}
          <div className="space-y-1.5 pb-px">
            <div className="h-[11px] w-[72px] rounded bg-muted-foreground/15 animate-pulse" />
            <div className="h-[9px] w-[52px] rounded bg-muted-foreground/15 animate-pulse" />
          </div>
        </div>
      </div>
      {/* Ticket card — 160 × (2/3) centred, matches real feed card */}
      <div className="flex justify-center px-4 mb-2">
        <div
      ...
      {/* Action bar — real icon shapes at skeleton opacity, matches gap-8 justify-center */}
      <div className="flex items-center justify-center gap-8 py-4">
        <Heart className="w-5 h-5 text-muted-foreground/20" />
        <MessagesSquare className="w-5 h-5 text-muted-foreground/20" />
        <Bookmark className="w-5 h-5 text-muted-foreground/20" />
        <Share2 className="w-5 h-5 text-muted-foreground/20" />
      </div>
    </div>
  );
}
