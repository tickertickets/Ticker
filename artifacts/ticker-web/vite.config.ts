...
export default defineConfig({
  ...
  plugins: [
    ...
    VitePWA({
      ...
      // null = do NOT inject any SW registration code into index.html.
      // We handle registration ourselves in main.tsx with a hadController guard
      // that prevents the unconditional controllerchange→reload that vite-plugin-pwa
      // would otherwise inject, which broke push-notification setup for new users
      // and users who cleared cache (the reload fired mid-subscription, killing it).
      injectRegister: null,
      strategies: "injectManifest",
      srcDir: "src",
      filename: "sw.ts",
      injectManifest: {
        // Precache only the app shell (HTML entry point) and static assets
        // (icons, badge). JS/CSS chunks are excluded intentionally:
        //   1. They are served from Vercel's CDN with immutable cache headers
        //      (Cache-Control: max-age=31536000, immutable) so the browser
        //      caches them on first download — no precaching needed.
        //   2. Precaching ALL chunks (vendor, tanstack, framer-motion, etc.)
        //      adds several MB to the SW install step. On mobile connections
        //      this pushes installation past the 8-second getSwRegistration()
        //      timeout in enablePushNotifications(), returning { reason: "no_sw" }
        //      and making push setup impossible for every new user until they
        //      refresh several times.
        //   3. Keeping the precache small means the SW activates in < 1 second
        //      on any connection, so push can always be set up on first visit.
